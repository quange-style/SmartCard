...
void CDlgDescind::OnBnClickedBnDescind()
{
	...
	GetDlgItemText(IDC_EDIT_AMOUNT, szTemp, sizeof(szTemp) - 1);
	lToDescind = atol(szTemp);

	GetDlgItemText(IDC_EDIT_BALANCE, szTemp, sizeof(szTemp) - 1);
	...
	do 
	{
		...
		memcpy(bData + 5,&g_Analyse.lBalance, 4);
		memset(bData + 9, '0', 16);

		GetDlgItemText(IDC_EDIT_LGCID, szTemp, sizeof(szTemp) - 1);
		...
		memcpy(&lendcint, bRecv + 7 + sizeof(RETINFO), 2);
		if (lendcint == 0)
		{
		...
		memcpy(&dcint, bRecv + 9 + sizeof(RETINFO), lendcint);
		...
		g_retInfo = g_Serial.Communicate(0, g_nBeep, 1, 47, NULL, 0, bRecv, sizeof(bRecv));
