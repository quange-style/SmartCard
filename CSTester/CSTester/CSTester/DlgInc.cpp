...
void CDlgInc::OnBnClickedBnIncrease()
{
	...
	do 
	{
		...
		memcpy(&lenchint, bRecv + 7 + sizeof(RETINFO), 2);
		if (lenchint == 0)
		{
		...
		// 发送充值申请，接收申请数据
		memcpy(&chint, bRecv + 9 + sizeof(RETINFO), lenchint);
		...
		memcpy(bData, &chapp, sizeof(chapp) - 4);

		if (g_bDeviceType == 2)
			g_retInfo = g_Serial.Communicate(0, g_nBeep, 1, 25, bData, sizeof(chapp) - 4, bRecv, sizeof(bRecv));
		else
			g_retInfo = g_Serial.Communicate(0, g_nBeep, 1, 43, bData, sizeof(chapp) - 4, bRecv, sizeof(bRecv));

		if (g_retInfo.wErrCode != 0)	break;

		memcpy(&lenps, bRecv + 7 + sizeof(RETINFO), 2);
		
		if (lenps > 0)
			memcpy(&ps, bRecv + 9 + sizeof(RETINFO), lenps);
	...
	((CCSTesterDlg * )AfxGetMainWnd())->DispResult(struct_Purse, &ps, lenps, "充值", g_liStart, g_liEnd);
