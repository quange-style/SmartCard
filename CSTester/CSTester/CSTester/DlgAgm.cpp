...
BEGIN_MESSAGE_MAP(CDlgAgm, CDialog)
	...
	ON_BN_CLICKED(IDC_BN_AGM_OPEN, &CDlgAgm::OnBnClickedBnAgmOpen)
...
void CDlgAgm::OnBnClickedBnEntry()
{
	...
	{
		printf("二维码进闸");
		((CCSTesterDlg * )AfxGetMainWnd())->DispResult(struct_ETicket, &etg, lenetg, "二维码进闸", g_liStart, g_liEnd);
	}
	else
...
void CDlgAgm::OnBnClickedBnExit()
{
	...
	if(bRfu[0] == 0x11)
		((CCSTesterDlg * )AfxGetMainWnd())->DispResult(struct_ETicket, &etg, lenetg, "二维码出闸", g_liStart, g_liEnd);
	else
		((CCSTesterDlg * )AfxGetMainWnd())->DispResult(struct_Purse, &ps, lenps, "出闸", g_liStart, g_liEnd);
}
...
void CDlgAgm::OnBnClickedBnAgmOpen()
{
	// TODO: 在此添加控件通知处理程序代码
