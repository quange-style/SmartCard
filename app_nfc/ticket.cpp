...
#define MI_OK 0
#define ERR_CARD_READ -1
#define ERR_CARD_NONE -1
#define ERR_INPUT_INVALID -1
...
//=================================================
int generate_crc8(uint8_t *pdata, uint8_t len, uint8_t key)
{
	return 0;
}
...
int sam_metro_active(int nsamsck, unsigned char * p_len_sam_id, unsigned char * p_sam_id, unsigned char * p_len_terminal_id, unsigned char * p_terminal_id)
{
	...
	do
	{
		////dbg_formatvar("nsamsck = [%d]",nsamsck);
		...
		//dbg_dumpmemory("95>>:",sztmp,sam_len);
		status = sam_cmd(nsamsck,sam_len,sztmp, sztmp, &sam_sw);
		//dbg_formatvar("sam_sw |= %04X",sam_sw);
		if (0x9000 != sam_sw || status <= 0)	break;

		//dbg_dumpmemory("95<<:", sztmp, status);
		...
		*p_len_sam_id = g_saminf[nsamsck].len_logical_id;
		...
		//dbg_formatvar("g_saminf[nsamsck].sam_logical_id + [%d][%d] = %s", g_saminf[nsamsck].offset_logical, g_saminf[nsamsck].len_logical_id,g_saminf[nsamsck].sam_logical_id);

		sam_len = 5;
		memcpy(sztmp, "\x00\xB0\x96\x00\x06",sam_len);
		//dbg_dumpmemory("96>>:",sztmp,sam_len);
		status = sam_cmd(nsamsck,sam_len,sztmp, sztmp, &sam_sw);
		//dbg_formatvar("sam_sw |= %04X",sam_sw);
		if (0x9000 != sam_sw || status <= 0)	break;

		//dbg_dumpmemory("96<<:",sztmp,status);
		...
		*p_len_terminal_id = g_saminf[nsamsck].len_terminal_id;
		...
		//dbg_formatvar("sam_sw |= %04X",sam_sw);
		if (0x9000 != sam_sw || status <= 0)	break;
		//dbg_dumpmemory("97<<:", sztmp, status);
		...
		if (nsamsck == 0)
		{
			for(int i=0; i<2; i++)
			{
				...
				//dbg_dumpmemory("B098>>:",sztmp,sam_len);
				status = sam_cmd(nsamsck,sam_len,sztmp, sztmp, &sam_sw);
				//dbg_dumpmemory("B098<<:", sztmp, status);
				if (0x9000 == sam_sw )
				{
...
//       0=成功
//========================================================
int sam_hst_active(int nsamsck, unsigned char * p_len_sam_id, unsigned char * p_sam_id, unsigned char * p_len_terminal_id, unsigned char * p_terminal_id)
{
	unsigned char status;
	unsigned char sztmp[256];
	unsigned short int sam_sw;
	...
	do
	{
		//dbg_formatvar("hst_sam_setbaud");
		sam_setbaud(nsamsck, SAM_BAUDRATE_38400);
		//dbg_formatvar("hst_sam_rst");
		status = sam_rst(nsamsck, sztmp);
		//dbg_formatvar("status = [%d]", status);
		if (status <= 0)	break;
		...
		printf("111  00 A4 00 00 02 3f 00 sam_sw=%x status=%d \n",sam_sw,status);
		status = sam_cmd(nsamsck, sam_len, sztmp, sztmp, &sam_sw);
		//dbg_formatvar("sam_sw |= %04X", sam_sw);
		if (status <= 0)	break;
		...
		printf("222  00 B0 95 00 0A sam_sw=%x status=%d \n",sam_sw,status);
		if (0x9000 != sam_sw || status <= 0)	break;
		...
		*p_len_sam_id = g_saminf[nsamsck].len_logical_id;
		...
		status = sam_cmd(nsamsck, sam_len, sztmp, sztmp, &sam_sw);
		//dbg_formatvar("sam_sw |= %04X", sam_sw);
		printf("333  00 B0 96 00 06 sam_sw=%x status=%d \n",sam_sw,status);
		if (0x9000 != sam_sw || status <= 0)	break;
		...
		*p_len_terminal_id = g_saminf[nsamsck].len_terminal_id;
		...
		////dbg_dumpmemory("1001:",sztmp,sam_len);
		status = sam_cmd(nsamsck, sam_len, sztmp, sztmp, &sam_sw);
		printf("444  00 A4 00 00 02 10 01 sam_sw=%x status=%d \n",sam_sw,status);
		////dbg_formatvar("sam_sw |= %04X",sam_sw);
		if (status <= 0)	break;
		...
		status = sam_cmd(nsamsck, sam_len, sztmp, sztmp, &sam_sw);
		//dbg_formatvar("sam_sw |= %04X", sam_sw);
		printf("555  00 B0 97 00 01 sam_sw=%x status=%d \n",sam_sw,status);
		if (0x9000 != sam_sw || status <= 0)	break;
		//dbg_dumpmemory("97<<:", sztmp, status);
		...
		if (nsamsck == 0)
		{
			for (int i = 0; i < 2; i++)
			{
				...
				memcpy(sztmp, "\x00\xB0\x98\x00\x04", sam_len);
				status = sam_cmd(nsamsck, sam_len, sztmp, sztmp, &sam_sw);
				printf("666  00 B0 98 00 04 sam_sw=%x status=%d \n",sam_sw,status);
				if (0x9000 == sam_sw)
				{
					g_saminf[nsamsck].dsn = sztmp[0];
					g_saminf[nsamsck].dsn <<= 8;
	...
	//dbg_formatvar("dsn = [%d]", g_saminf[nsamsck].dsn);
...
int octjtb_saminit(int nsamsck, unsigned char * p_len_sam_id, unsigned char * p_sam_id, unsigned char * p_len_terminal_id, unsigned char * p_terminal_id)
{
	...
	do
	{
		...
		printf("111 00A40000023F00 sam_sw=%x status=%d \n",sam_sw,status);
		if (0x9000 != sam_sw || status <= 0 )	break;

		sam_len = 5;
		memcpy(sztmp, "\x00\xB0\x95\x00\x00",sam_len);
		//dbg_dumpmemory("00B0:",sztmp,sam_len);
		status = sam_cmd(nsamsck,sam_len,sztmp, sztmp, &sam_sw);
		
		printf("222 00B0950000 sam_sw=%x status=%d \n",sam_sw,status);
		//dbg_formatvar("sam_sw |= %04X",sam_sw);
		//dbg_dumpmemory("sztmp-resp",sztmp,status);
		if (0x9000 != sam_sw || status <= 0)	break;
		...
		*p_len_sam_id = g_saminf[nsamsck].len_logical_id;
		...
		status = sam_cmd(nsamsck,sam_len,sztmp, sztmp, &sam_sw);
		printf("333 00B0960006 sam_sw=%x status=%d \n",sam_sw,status);
		//dbg_formatvar("sam_sw |= %04X",sam_sw);
		//dbg_dumpmemory("sztmp-resp",sztmp,status);
		if (0x9000 != sam_sw || status <= 0)	break;
		...
		*p_len_terminal_id = g_saminf[nsamsck].len_terminal_id;
		...
		memcpy(g_saminf[nsamsck].sam_logical_id,"\x00\x00",2);
		memcpy(&g_saminf[nsamsck].sam_logical_id[2], sztmp, g_saminf[nsamsck].len_terminal_id);
		*p_len_sam_id = g_saminf[nsamsck].len_logical_id;
		memcpy(p_sam_id,"\x00\x00",2);
		memcpy(&p_sam_id[2], g_saminf[nsamsck].sam_terminal_id, g_saminf[nsamsck].len_terminal_id);
		//dbg_dumpmemory("jtb_p_sam_id =",p_sam_id,g_saminf[nsamsck].len_logical_id);
		...
		//dbg_dumpmemory("sztmp=",sztmp,7);
		status = sam_cmd(nsamsck,sam_len,sztmp, sztmp, &sam_sw);
		printf("444 00A40000028011  sam_sw=%x status=%d \n",sam_sw,status);
		//dbg_formatvar("sam_sw |= %04X",sam_sw);
		//dbg_dumpmemory("sztmp-resp",sztmp,status);
		if ( status <= 0)	break;
		...
		//dbg_dumpmemory("sztmp=",sztmp,5);
		status = sam_cmd(nsamsck,sam_len,sztmp, sztmp, &sam_sw);
		printf("555 00B0970001 sam_sw=%x status=%d \n",sam_sw,status);
		//dbg_formatvar("sam_sw |= %04X",sam_sw);
		//dbg_dumpmemory("sztmp-resp",sztmp,status);
		if (0x9000 != sam_sw || status <= 0)	break;
...
int searchcard(unsigned char *lpcardno)
{
	...
	for (int i=0;i<1;i++)
	{
		printf("searchcard i %d\n",i);
		if (ISO14443A_Request(0x52, tmp) == 0)
		{
			printf("ISO14443A_Request i=%d value %#x %#x\n", i, tmp[0],tmp[1]);
			if (tmp[0] != 0x44)
			{	
				if (ISO14443A_Anticoll(snr) == 0)
				{
					printf("ISO14443A_Anticoll i=%d value %#x %#x %#x %#x\n", i, snr[0],snr[1],snr[2],snr[3]);
					if (ISO14443A_Select(snr, tmp) == 0)
					{
						
						card_type[i] = (tmp[0] & 0x24) == 0x20 ? 0x03 : 0x02;
						memcpy(ack[i], snr, 4);
						printf("ISO14443A_Select i=%d value %#x\n", i, tmp[0]);
					}
				}
			}
			else
...
//	uint8_t key[16] = {0};
//
//	////dbg_dumpmemory("Api::qr_readbuf--",Api::qr_readbuf,112);
//	//dbg_formatvar("search_qr_card_ex");
//
//	if((Api::qr_readbuf[0] == 0x51)&&(Api::qr_readbuf[1] == 0x52))
//	{
//		////dbg_formatvar("search_card_ex=%02x%02x",Api::qr_readbuf[0],Api::qr_readbuf[1]);
//		////dbg_formatvar("search_card_ex=%02x%02x",Api::qr_readbuf[2],Api::qr_readbuf[3]);
...
//	else
//	{
//		//dbg_formatvar("search_qr_card_ex---ERR_CARD_NONE");
//		ret = ERR_CARD_NONE;
//	}
//
//
...
//
//	if((Api::qr_readbuf[7] == 0x80))	//二维码
//	{
//		memcpy(tmp_package_time, Api::qr_readbuf, 7);
//		TimesEx::bcd_time_calculate(tmp_package_time, T_TIME, 0, 0, 0, 5);//范围两秒内的都认
...
//		}else{
//			type = 0x08;
//			memcpy(Api::qr_readbuf, &Api::qr_readbuf[7], Api::qr_readbuf_len);
//		}
//
//	}else
//	{
...
//			ret = ERR_CARD_NONE;
//		}else{
//			//dbg_formatvar("Api::qr_readbuf_len = %d",Api::qr_readbuf_len);
//			memcpy(Api::qr_readbuf, &Api::qr_readbuf[7], Api::qr_readbuf_len);
//			//dbg_dumpmemory("Api::qr_readbuf = ",Api::qr_readbuf,Api::qr_readbuf_len);
//			type = 0x11;
//		}
//
//	}
...
//		}
//
//	}else if((Api::qr_readbuf[7] == 0x72)&&(Api::qr_readbuf[8] == 0x6D))//健康码
//	{
//		memcpy(tmp_package_time, Api::qr_readbuf, 7);
//		TimesEx::bcd_time_calculate(tmp_package_time, T_TIME, 0, 0, 0, 5);//范围两秒内的都认
//		if(memcmp(Api::get_package_time, tmp_package_time, 7) > 0)//说明该码是之前扫的，判断为无效
...
//		}
//
//	}else if((Api::face_readbuf[7] == 0x02)&&(Api::face_readbuf[8] == 0x30)&&(Api::face_readbuf[9] == 0x30)&&(Api::face_readbuf[10] == 0x63)&&(Api::face_readbuf[11] == 0x34))	//人脸
//	{
//		memcpy(tmp_package_time, Api::face_readbuf, 7);
//		TimesEx::bcd_time_calculate(tmp_package_time, T_TIME, 0, 0, 0, 5);//范围两秒内的都认
//		if(memcmp(Api::get_package_time, tmp_package_time, 7) > 0)//说明该码是之前扫的，判断为无效
//		{
//			//g_Record.log_buffer("auth code is invalid ,valid date is over two second:", tmp_package_time, 7);
...
//			ret = ERR_CARD_NONE;
//		}else{
//			memcpy(Api::face_readbuf, &Api::face_readbuf[7], Api::face_readbuf_len);
//			type = 0x09;
//		}
//
//	}
...
//		for (int i=0;i<2;i++)
//		{
//			//dbg_formatvar("ISO14443A_Request_begin");
...
//		////g_Record.log_out(0, level_error, "type=%d",type);
//
//		if (type == 3)
//		{
...
// 		ulen = MifareProCom(ulen, sztmp, &response_sw);
	...
	printf("nresult=%d response_sw=%x \n",nresult,response_sw);
	return nresult;
}
...
unsigned short search_jtbcpu(unsigned char * p_pid, unsigned char& type)
{
	...
	do
	{
		...
		if( (ulen == 0) || (response_sw != 0x9000))
		{
			...
			if( (ulen == 0) || (response_sw != 0x9000))
			{
				//g_Record.log_out(ulen, level_error, "select interconnecte elec wallet failed");
...
unsigned short distinguish_cpu_ex(unsigned char * p_pid, unsigned char& type)
{
	...
	unsigned char cardsubtype = 0;
	unsigned char industry[2] = { 0 };
	...
	do
	{
		...
		ulen = 5;
		memcpy(sztmp, "\x00\xB0\x85\x00\x00", ulen);
		...
		if (response_sw==0x9000)
		{
			...
			memcpy(industry, sztmp + 4, 2);
			//dbg_dumpmemory("industry =", industry, 2);
			if (memcmp(industry, "\x00\x00", 2) == 0)//红山通
			{
				if(memcmp(sztmp, "\x09\x91", 2) == 0)
					type = 4;				//次票
				else
					nresult = ERR_CARD_NONE;
			}
			else if(memcmp(industry, "\x00\x03", 2) == 0){
				cardtype = sztmp[16];//票卡主类型
				cardsubtype = sztmp[17];//票卡子类型
				//dbg_formatvar("cardtype = %02x%02x",cardtype, sztmp[17]);
				if (cardtype == 0x03)
					type = 6;				//次票
				else if(cardtype == 0x08){//云卡
					if(sztmp[17] == 0x00)
						type = 3;				//地铁
					else
						type = 6;				//次票
				}
				else if((cardtype == 0x04) && ((cardsubtype == 0x01)||(cardsubtype == 0x03)))
					type = 6;				//次票
				else
					type = 3;				//地铁
			}else{
		...
		else
		{
			...
			////g_Record.log_buffer("pse = ",sztmp,ulen);
 			ulen = MifareProCom(ulen, sztmp, &response_sw);
			//dbg_formatvar("response_sw = %04x",response_sw);
			...
			{
				//g_Record.log_out(0, level_error,"ppse no response");
				nresult = response_sw;
				break;
			}else if((response_sw!=0x9000))
			{
				//g_Record.log_out(0, level_error,"ppse response_sw = %04x",response_sw);
				nresult = ERR_CARD_NONE;
				break;
			}
...
int svt_readrecord(unsigned char aid, unsigned char record_no, unsigned char len, unsigned char *lpdata)
{
	...
	if(sztmp[2] == 0x11)
	{
		...
		sztmp[3]= ((aid << 3) & 0xF8);
	sztmp[4]= len;
	//dbg_dumpmemory("svt_readrecord =",sztmp, ulen);
...
int svt_getonlineserial(int nsamsck,unsigned char *lpserial)
{
	...
	do
	{
		...
		{
		ulen = 11;
		memcpy(sztmp, "\x00\x20\x00\x00\x06\x55\x72\x75\x6D\x71\x69", ulen);
		//dbg_dumpmemory("svt_getonlineserial =", sztmp, ulen);
		response_len = MifareProCom(ulen,sztmp, &response_sw);
		//dbg_dumpmemory("svt_getonlineserial |=", sztmp, response_len);
...
int svt_recharge(unsigned char trType, unsigned char * p_timenow, int nsamsck, int nstep, unsigned long value, unsigned char *lpinf)
{
	...
	//g_Record.log_out(respone_len, level_error,"svt_recharge, nstep=%d, nresult=%d, sam_sw=%d, respone_len=%d", nstep, nresult, sam_sw, respone_len);
								 ...
								 unsigned char *p_public_base, unsigned char *metro,unsigned char *his_all,long lAmount,long lbalance, unsigned char *respone_data)
{
	...
	do
	{
		...
		memcpy(sztmp+ulen, g_saminf[nsamsck].sam_terminal_id, g_saminf[nsamsck].len_terminal_id);
		ulen += g_saminf[nsamsck].len_terminal_id;
		//dbg_dumpmemory("sztmp = ",sztmp,ulen);
		response_len = MifareProCom(ulen, sztmp, &sam_sw);
		//dbg_formatvar("sam_sw |= %04X",sam_sw);
		////dbg_dumpmemory("sztmp_resp = ",sztmp,response_len);
		if ((response_len==0) || (0x9000!=sam_sw))
		{
			nresult = 3;
			break;
		...
		////dbg_dumpmemory("lpinitconsume = ",sztmp,response_len);

		lpinitconsume = (LPTINITCONSUME)sztmp;
		PsamGetMac1.encryver = lpinitconsume->encryver;
		PsamGetMac1.calcmarker = 0x00;//lpinitconsume->calcmarker;
		memcpy(PsamGetMac1.offlinedealserial, lpinitconsume->offlinedealserial, 2);
		memcpy(PsamGetMac1.random, lpinitconsume->random, 4);
		...
		////dbg_dumpmemory("lplogicnumber = ",p_public_base+12,8);
		...
		memcpy(sztmp, "\x80\xDC\x02\xD0\x80", 5);
		if (lAmount == 0)
			metro[94+8]    = 0x01;         // 交易类型 ,  00：初始值,01：EC入站/上车/进场等；02：EC出站/下车/出场等；
		else
			metro[94+8]    = 0x02;         // 交易类型 ,  00：初始值,01：EC入站/上车/进场等；02：EC出站/下车/出场等；
		//memcpy(metro,"\x27\x02\x7D\x01",4);
		metro[95+8] = Api::current_station_id>>8;
		metro[96+8] = Api::current_station_id&0xFF;
		memcpy(metro+105,p_time_now,7);
		...
		*/
		...
		//if(lAmount > 0)//出闸交易才写记录文件
		{
			his_all[1] = 0x00;
			his_all[2] = 0x00;
			memcpy(his_all + 3, g_saminf[nsamsck].sam_terminal_id, 6);
...
//       0=成功
//========================================================
uint16_t svt_hst_lock_card(int nsamsck,unsigned char *lplogicnumber)
{
	unsigned char respone_len;
	unsigned char sam_len;
	unsigned char sztmp[256];
	...
	do
	{
		sam_len = 5;
		memcpy(sztmp, "\x00\x84\x00\x00\x04",5);
		//dbg_dumpmemory("sztmp_send = ",sztmp,sam_len);
		////g_Record.log_out(0, level_error, "getrandom:%02X%02X%02X%02X%02X",sztmp[0],sztmp[1],sztmp[2],sztmp[3],sztmp[4]);
		respone_len = MifareProCom(sam_len,sztmp, &sam_sw);
		////g_Record.log_out(0, level_error, "sam_sw:%04X",sam_sw);
		//dbg_dumpmemory("sztmp_resp = ",sztmp,respone_len);
		...
		memcpy(sztmp+sam_len, lplogicnumber, 8);//卡应用序列号
		sam_len += 8;
								 ...
								 unsigned char *dim_record_id, unsigned char *dim_lock, unsigned char *dim_lpfile, unsigned char *respone_data)
{
	...
	do
	{
		...
		for (i=0;i<recordcnt;i++)
		{
			...
			//dbg_dumpmemory("80DC =",sztmp,ulen);
			response_len = MifareProCom(ulen, sztmp, &sam_sw);
			//dbg_formatvar("[%d]80DC |= %04X",i,sam_sw);
		...
		//dbg_dumpmemory("8070 =",sztmp,ulen);
		response_len = sam_cmd(nsamsck,ulen,sztmp, sztmp, &sam_sw);
		//dbg_formatvar("8070 |= %04X",sam_sw);
		//dbg_dumpmemory("8070 resp=",sztmp,response_len);
		if ((response_len==0) || (0x9000!=sam_sw))
		{
			nresult = 3;
			break;
		...
		response_len = sam_cmd(nsamsck,ulen,sztmp, sztmp, &sam_sw);
		//dbg_formatvar("8072 |= %04X",sam_sw);
		//dbg_dumpmemory("8072 resp=",sztmp,response_len);
		if ((response_len==0) || (0x9000!=sam_sw))
		{
			nresult = 5;
			break;
...
//       0=成功
//========================================================
uint16_t svt_updata_more_complex_hst_dis(unsigned char tac_type, unsigned char * p_time_now, int nsamsck,
								 unsigned char *lplogicnumber, long lAmount, unsigned char recordcnt,
								 unsigned char *dim_record_id, unsigned char *dim_lock, unsigned char *dim_lpfile, unsigned char *discount_lpfile, unsigned char *respone_data)
{
	unsigned char sztmp[256];
	unsigned char szmac[8];
	unsigned char ulen;
	...
	do
	{
		...
		//if (discount_lpfile != NULL)
		{

			memcpy(sztmp, "\x80\xDC\x07\xB8\x0C", 5);

			sztmp[4] = discount_lpfile[0];

			//dbg_formatvar("sztmp |= %02X",discount_lpfile[0]);

			//dbg_dumpmemory("discount_lpfile =",discount_lpfile,48);

			memcpy(sztmp+5, discount_lpfile + 1, discount_lpfile[0]);

			//ulen = 5 + discount_lpfile[0];

			//memcpy(sztmp+5, discount_lpfile + 1, 12);
			...
			response_len = MifareProCom(ulen, sztmp, &sam_sw);
			//dbg_formatvar("80DC |= %04X",sam_sw);
			//dbg_dumpmemory("80DC resp=",sztmp,response_len);
...
//       0=成功
//========================================================
uint16_t svt_updata_more_complex_hst(unsigned char tac_type, unsigned char * p_time_now, int nsamsck,
								 unsigned char *lplogicnumber, long lAmount, unsigned char recordcnt,
								 unsigned char *dim_record_id, unsigned char *dim_lock, unsigned char *dim_lpfile, unsigned char *discount_lpfile, unsigned char *respone_data)
{
	unsigned char sztmp[256];
...
int get_cpu_last_tac(uint16_t off_line_counter, uint8_t tac_type, unsigned char * p_recv)
{
	...
	do
	{
		////g_Record.log_out(0,level_error,"get_cpu_last_tac off_line_counter:%d",off_line_counter);

		if (svt_selectfile(0x3F01) < 0)
		{
			ret = -1;
			////dbg_formatvar("get_cpu_last_tac | svt_selectfile(3F01) failed in function get_cpu_last_tac");
		...
		len_cmd += 2;
		//dbg_dumpmemory("get tac send=",data_cmd,7);
		////g_Record.log_out(0, level_error, "get tac send:%02X%02X%02X%02X%02X%02X%02X", data_cmd[0], data_cmd[1], data_cmd[2], data_cmd[3], data_cmd[4], data_cmd[5], data_cmd[6]);
		////g_Record.log_buffer((char *)("get tac send:"), data_cmd, 7);
		len_recv = MifareProCom(len_cmd, data_cmd, &sw_result);
		////g_Record.log_out(0, level_error, "sw_result=%04X",sw_result);
		//dbg_dumpmemory("svt_selectfile |=",data_cmd,len_recv);
