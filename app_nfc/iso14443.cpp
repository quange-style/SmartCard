...
int rfdev_init(void)
{
	...
	if (g_hostreader > 0)
	{
		...
		printf("open /dev/rf531dev0 error\n");
	}
...
int antenna_switch(int idx)
{
	...
	printf("antenna_switch int idx %d\n", idx);
...
char ISO14443A_Request(unsigned char req_code, unsigned char *atq)
{
	...
	for(int i = 0;i<1;i++)
	{
		...
		//dbg_formatvar("ISO14443A_Request:%d,%02X",i,status);
...
unsigned char MifareProRst(unsigned char cParameter,unsigned char cpLength,unsigned char *cpOutMsg)
{
    ...
    if (status==0x00)
    {
		...
		//dbg_dumpmemory("cpOutMsg = ",cpOutMsg,len);
    }
...
unsigned char MifareProCom(unsigned char cpLength,unsigned char *lpdata, unsigned short int *respone_sw)
{
	...
	//g_Record.log_buffer("MifareProCom send= ",lpdata,cpLength);

	*respone_sw=sw;
	printf("MifareProCom 111\n");
	...
	printf("MifareProCom 222 status=%d \n",status);
    if (status==0x00)
    {
        len = buf[0];
	...
	/*
	if(status != 0)
		g_Record.log_buffer("MifareProCom resp= ",lpdata,status);
	g_Record.log_out(0, level_disaster,"MifareProCom resp = %04X",sw);
	*/
