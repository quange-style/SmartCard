...
int readul(int nsamsck, unsigned char *lpbuffer)
{
    ...
    //		dbg_dumpmemory((char *)"page1=",sjt_card.data, 16);
    //		dbg_dumpmemory((char *)"page2=",sjt_card.data+16, 16);
    //		dbg_dumpmemory((char *)"page3=",sjt_card.data+32, 16);
    //		dbg_dumpmemory((char *)"page4=",sjt_card.data+48, 16);
...
int writeul(int nsamsck, int nzone, unsigned char *lpbuffer)
{
    ...
    //写数据
    if ( nzone == 0) offset_pager = USERZONE_PAGER1;
    else
...
//       0=成功
//========================================================
int svt_recharge(int nsamsck, int nstep, unsigned long value, unsigned char *lpinf)
{
...
//				dbg_dumpmemory("recharge=",lpinf,22);
...
//       0=成功
//========================================================
int svt_consum(int nsamsck, unsigned long value,unsigned char trtype, unsigned char *lpmetrofile, unsigned char *respone_data)
{
    unsigned char sztmp[256];
...
//       0=成功
//===============================================================
int svt_update_binfile(int nsamsck,int file_id, int offset, int nlen, unsigned char *lpinf)
{
...
//			dbg_dumpmemory("0084_S=",sztmp,sam_len);
        respone_len = MifareProCom(sam_len,sztmp, &sam_sw);
//			dbg_dumpmemory("0084_R=",sztmp,respone_len);
        ...
        memcpy(sztmp+sam_len, g_metroinf.logicnumber, 8);
        sam_len += 8;
//			dbg_dumpmemory("801A_S=",sztmp,sam_len);
        respone_len = sam_cmd(nsamsck, sam_len, sztmp, sztmp, &sam_sw);
//			dbg_dumpmemory("801A_R=",sztmp,respone_len);
        if (( respone_len==0 ) || (0x9000!=sam_sw))
        {
            nresult = -1;
            break;
...
//			dbg_dumpmemory("80FA_S=",sztmp,sam_len);
        respone_len = sam_cmd(nsamsck, sam_len, sztmp, sztmp, &sam_sw);
//			dbg_dumpmemory("80FA_R=",sztmp,respone_len);
        if (( respone_len==0 ) || (0x9000!=sam_sw))
        {
            nresult = -1;
            break;
...
//			dbg_dumpmemory("04D6_S=",sztmp,sam_len);
        respone_len = MifareProCom(sam_len,sztmp, &sam_sw);
//			dbg_dumpmemory("04D6_R=",sztmp,respone_len);
...
//       0=成功
//========================================================
int svt_updata_complex(int nsamsck, unsigned char *lplogicnumber, unsigned char record_id, unsigned char lock, unsigned char *lpfile, unsigned char *respone_data)
{
    unsigned char sztmp[256];
    unsigned char szmac[8];
    unsigned char ulen;
...
//		dbg_dumpmemory("8050-",sztmp,response_len);
        ...
        sztmp[ulen++] = _METROFILE_SIZE_;
        sztmp[ulen++] = lock;
        memcpy(sztmp+ulen, lpfile, 45);
