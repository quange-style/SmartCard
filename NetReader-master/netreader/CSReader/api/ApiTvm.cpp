...
uint8_t nlocked_analyse_id[20] = { 0 };
...
void Api::Tvm_TicketAnalyze(uint8_t * param_stack, uint8_t * data_to_send, uint16_t& len_data)
{
	...
	do
	{
		...
		g_Record.log_out(0, level_normal,"analyse_common... ...");
		ret = p_ticket->analyse_common(2, &md_effect, &ticket_prm, &locked, len_lock, &analyse, NULL);
		if (ret.wErrCode != 0)	break;
		
		g_Record.log_out(0, level_normal,"analyse_free_zone... ...");
...
void Api::Tvm_GetETicket_Sale(uint8_t * param_stack, uint8_t * data_to_send, uint16_t& len_data)
{
	...
	do
	{
		...
		ret.wErrCode = TicketBase::search_card(&p_ticket,NULL, NULL, NULL, operSale);
		if (ret.wErrCode != 0)	break;
...
void Api::Tvm_ThirdPay_Sale(uint8_t * param_stack, uint8_t * data_to_send, uint16_t& len_data)
{
	...
	do
	{
		...
		gettimeofday(&start,NULL);
        int rfs_status= rfdev_get_rfstatus(1);//在发售前首先对主天线板进行复位
		gettimeofday(&end,NULL);

		time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒
		msec = time_use / 1000;
		g_Record.log_out(ret.wErrCode, level_warning,"rfdev_get_rfstatus use time = [%ld],[%ld]",msec,rfs_status);
...
void Api::Tvm_GetETicketInfo(uint8_t * param_stack, uint8_t * data_to_send, uint16_t& len_data){
    common_getElectQrMsg(param_stack, data_to_send, len_data);
}
