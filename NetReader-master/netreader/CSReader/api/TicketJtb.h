...
protected:
	...
	METRO_JTBCPU_INF m_ReadInf;

	uint16_t metro_jtb_read(P_METRO_JTBCPU_INF p_mtr_inf);

	METRO_JTBCPU_INF file_need_read(ETYTKOPER type, uint8_t * p_buffer);
	METRO_JTBCPU_INF file_need_write(ETYTKOPER type, P_METRO_JTBCPU_INF p_read_inf);
	...
	RETINFO write_card(ETYTKOPER operType, METRO_JTBCPU_INF write_inf, uint8_t tac_type, long trade_amount, long& sam_seq, char * p_trade_tac);
	uint16_t write_card(uint8_t tac_type, uint8_t * p_app_ctrl, int record_id, uint8_t lock_flag, uint8_t * p_file_data, long& sam_seq);
	RETINFO lock_card(unsigned char *lplogicnumber, unsigned char *issueno, long& sam_seq, char * p_trade_tac);
