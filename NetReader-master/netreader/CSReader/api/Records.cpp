...
LOG_LEVEL Records::current_log_level		= level_disaster;
...
void Records::ergodic_dir_delete_target(char * p_path_dir, uint8_t * p_today, int save_time, int date_offset_filename)
{
	...
	if (access(p_path_dir, 0) == 0)
	{
		...
		while((ent = readdir(pDir)) != NULL)
		{
			...
			{
				unsigned long lsize = 1024 * 1024 * 50;//暂时给到50M  文件夹大小超过50M则会被删除
				if (GetLogDirSize(NAME_LOG_FOLDER) >= lsize)
				{
					for (int n = -28;n<0;n++)
					{
						g_Record.log_out_debug(0, level_normal, "[GetLogDirSize] = %d",(int)GetLogDirSize(NAME_LOG_FOLDER));
						...
						g_Record.log_out_debug(0, level_normal, "[pathname_log = %s, [n] = %d]",pathname_log,n);
						remove(pathname_log);
						g_Record.log_out_debug(0, level_normal, "[GetDirSize111] = %ld,%d",GetLogDirSize(NAME_LOG_FOLDER),n);
						if (GetLogDirSize(NAME_LOG_FOLDER) < lsize)
						{
							break;
						}
