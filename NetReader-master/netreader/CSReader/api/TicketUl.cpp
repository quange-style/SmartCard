...
TicketUl::TicketUl(char * p_current_sam, char * p_sam_posid, uint8_t * physic_info)
{
    ...
    pMemoryPos = ((uint8_t *)this) + sizeof(TicketUl);
...
RETINFO TicketUl::write_card(ETYTKOPER operType, long trade_amount, long& sam_seq, char * p_trade_tac)
{
	...
	do
	{
		...
		for (write_count=0;write_count<2;write_count++)
		{
            ...
            g_Record.log_out(0, level_error, "writeul_new = [%04X]",(write_count+1), ret.wErrCode);
			if (ret.wErrCode == 0)
			{
...
RETINFO TicketUl::entry_gate(PENTRYGATE p_entry,PETICKETDEALINFO p_eticketdealinfo,char cOnlineRes)
{
    RETINFO ret		= {0};
	long lsamseq	= 0;
...
RETINFO TicketUl::exit_gate(PPURSETRADE p_exit,PETICKETDEALINFO p_eticketdealinfo, MODE_EFFECT_FLAGS * p_degrade_falgs,char cOnlineRes)
{
    RETINFO ret		= {0};
	long lsamseq	= 0;
...
RETINFO TicketUl::check_online(ETYTKOPER operType,PETICKETAUTHINFO p_eticketauthinfo)
{
	RETINFO ret				= {0};
	return ret;
}
...
RETINFO TicketUl::sale(PSJTSALE p_sjt_sale, uint16_t& len_sjt, POTHERSALE p_svt_sale, uint16_t& len_svt, TICKET_PARAM& ticket_prm)
{
	...
	do
	{
		...
		if (ret.wErrCode == 0)  len_sjt = sizeof(SJTSALE);
...
RETINFO TicketUl::exit_sjt_sale(PSJTSALE p_exit_sjt_sale)
{
	...
	p_exit_sjt_sale->bPaymentMeans = 0x05;	// 行政处理收费
