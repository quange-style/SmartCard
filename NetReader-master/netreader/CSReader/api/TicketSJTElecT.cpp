...
TicketSJTElecT::TicketSJTElecT(char * p_current_sam, char * p_sam_posid, uint8_t * physic_info)
{
	...
	pMemoryPos = ((uint8_t *)this) + sizeof(TicketSJTElecT);
...
uint16_t TicketSJTElecT::metro_elec_read(P_METRO_ELECT_INF p_mtr_inf)
{
	uint16_t u_ret			= 0;
...
RETINFO TicketSJTElecT::analyse_common(uint8_t wk_area,MODE_EFFECT_FLAGS * p_degrade_falgs,
        TICKET_PARAM * p_ticket_prm,
        PTICKETLOCK p_lock, uint16_t& lenlock, PBOMANALYZE p_analyse, char * p_mode)
{
	RETINFO ret				= {0};
	return ret;
}

RETINFO TicketSJTElecT::analyse_work_zone(MODE_EFFECT_FLAGS * p_degrade_falgs,
        TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse, PPURSETRADE p_purse, PBOMETICANALYZE p_eticanalyse)
{
    RETINFO ret				= {0};
    char exit_mode				= 0;
    ...
    switch (m_ticket_data.simple_status){
        case SStatus_Entry://已经进站
            ...
            result_prm = g_Parameter.query_fare(CmdSort::m_time_now, m_ticket_data.fare_type,
                                                ...
                                                m_ticket_data.p_entry_info->time, &trade_val);
            if (result_prm){
            ...
            if(memcmp(CmdSort::m_time_now, validOutTime, 7) > 0)
            {
                ...
                if (p_eticanalyse != NULL)
                {
					...
					//超时罚款  线网最高票价-车资(储值票需要打折扣)
                    p_eticanalyse->lPenalty1=penalty_val_overtime - trade_val;
                }
                break;
            }

            //2，超乘 储值票不需要判断超乘问题 但是如果是预付费，需要判断余额，余额不足的情况需要返回余额不足状态码
            ...
            //如果是异常情况，不开闸，没有计算，反之进行金额的计算，开闸时需要计算应收金额
            if (p_purse != NULL)
            {
...
RETINFO TicketSJTElecT::entry_gate(PENTRYGATE p_entry,PETICKETDEALINFO p_eticketdealinfo,char cOnlineRes)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	uint16_t length = 0;
...
RETINFO TicketSJTElecT::exit_gate(PPURSETRADE p_exit,PETICKETDEALINFO p_eticketdealinfo, MODE_EFFECT_FLAGS * p_degrade_falgs,char cOnlineRes)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
RETINFO TicketSJTElecT::svt_increase(PPURSETRADE p_purse, uint8_t * p_time, uint8_t * p_mac2,uint8_t paytype)
{
	RETINFO ret					= {0};
	return ret;
}

RETINFO TicketSJTElecT::svt_decrease(PPURSETRADE p_purse, uint8_t u_pay_flag,uint8_t paytype)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
METRO_ELECT_INF TicketSJTElecT::file_need_read(ETYTKOPER type, uint8_t * p_buffer)
{
	int ptr_buffer			= 0;
...
METRO_ELECT_INF TicketSJTElecT::file_need_write(ETYTKOPER type, P_METRO_ELECT_INF p_read_inf)
{
	METRO_ELECT_INF ret_inf = {0};
...
void TicketSJTElecT::get_qrInfo(uint8_t * p_grInfo)
{
	...
	if (m_ticket_data.qr_ticket_status == STATUS_INIT_SALE)//初始发码状态
	{
		//上次交易类型
		memcpy(m_ticket_data.qr_ticketdata.lasttredeType, p_grInfo+len_getData, 2);
		len_getData+=2;
		//上次交易时间
		memcpy(m_ticket_data.qr_ticketdata.lastTradeTime, p_grInfo+len_getData, 14);
		len_getData+=14;
		//中心校验码 mac
		...
		len_getData+=14;
		g_Record.log_out(0, level_error,"get_qrInfo(mac=%s)", m_ticket_data.qr_ticketdata.mac);
		g_Record.log_out(0, level_error,"get_qrInfo(entryDeviceType=%s)", m_ticket_data.qr_ticketdata.entryDeviceType);
	...
	memcpy(m_ticket_data.logical_type, "\x02\x00", 2);//储值票
    memcpy(m_ticket_data.fare_type, m_ticket_data.logical_type, 2);
}
...
ElectTicketType TicketSJTElecT::getCurrentTicketType(){
    return ELECT_TICKET_PRE;//电子预付费储值票
}
