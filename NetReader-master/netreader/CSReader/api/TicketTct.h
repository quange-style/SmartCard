...
}METRO_TCT_CPU_INF, * P_METRO_TCT_CPU_INF;
...
typedef struct
{
	...
	bool	need_active;			// 多日票是否需要激活

}FMDATA;

class TicketTct : public TicketBase
{
...
public:
	TicketTct(void);
	TicketTct(char * p_current_sam, char * p_sam_posid, uint8_t * physic_info);
	~TicketTct(void);
...
protected:
	...
	METRO_TCT_CPU_INF m_ReadInf;
	FMDATA m_fm_data; 

	uint16_t metro_svt_read(P_METRO_TCT_CPU_INF p_mtr_inf);

	METRO_TCT_CPU_INF file_need_read(ETYTKOPER type, uint8_t * p_buffer);
	METRO_TCT_CPU_INF file_need_write(ETYTKOPER type, P_METRO_TCT_CPU_INF p_read_inf);
	...
	RETINFO write_card(ETYTKOPER operType, METRO_TCT_CPU_INF write_inf, uint8_t tac_type, long trade_amount, long& sam_seq, char * p_trade_tac);
