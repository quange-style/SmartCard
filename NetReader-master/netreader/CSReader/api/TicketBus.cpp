...
TicketBus::TicketBus(char * p_current_sam, char * p_sam_posid, uint8_t * physic_info)
{
	...
	pMemoryPos = ((uint8_t *)this) + sizeof(TicketBus);
...
RETINFO TicketBus::entry_gate(PENTRYGATE p_entry,PETICKETDEALINFO p_eticketdealinfo,char cOnlineRes)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
RETINFO TicketBus::exit_gate(PPURSETRADE p_exit,PETICKETDEALINFO p_eticketdealinfo, MODE_EFFECT_FLAGS * p_degrade_falgs,char cOnlineRes)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
RETINFO TicketBus::check_online(ETYTKOPER operType,PETICKETAUTHINFO p_eticketauthinfo)
{
	RETINFO ret				= {0};
	return ret;
}
...
RETINFO TicketBus::svt_decrease(PPURSETRADE p_purse, uint8_t u_pay_flag,uint8_t paytype)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
