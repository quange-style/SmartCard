...
TicketSvt::TicketSvt(char * p_current_sam, char * p_sam_posid, uint8_t * physic_info)
{
	...
	pMemoryPos = ((uint8_t *)this) + sizeof(TicketSvt);
...
RETINFO TicketSvt::analyse_work_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse, PPURSETRADE p_purse, PBOMETICANALYZE p_eticanalyse)
{
	RETINFO ret				= {0};
...
RETINFO TicketSvt::analyse_free_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse, PBOMETICANALYZE p_eticanalyse)
{
	RETINFO ret				= {0};
...
RETINFO TicketSvt::entry_gate(PENTRYGATE p_entry,PETICKETDEALINFO p_eticketdealinfo,char cOnlineRes)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
RETINFO TicketSvt::exit_gate(PPURSETRADE p_exit,PETICKETDEALINFO p_eticketdealinfo, MODE_EFFECT_FLAGS * p_degrade_falgs,char cOnlineRes)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
RETINFO TicketSvt::check_online(ETYTKOPER operType,PETICKETAUTHINFO p_eticketauthinfo)
{
	RETINFO ret				= {0};
	return ret;
}

RETINFO TicketSvt::esale(PSJTSALE p_sjt_sale, uint16_t& len_sjt, PETICKETDEALINFO p_eticketdealinfo, uint16_t& len_eticket_sale, TICKET_PARAM& ticket_prm)
{
	RETINFO ret				= {0};
	return ret;
}

RETINFO TicketSvt::thirdesale(PSJTSALE p_sjt_sale, uint16_t& len_sjt, PETICKETDEALINFO p_eticketdealinfo, uint16_t& len_eticket_sale, TICKET_PARAM& ticket_prm , char * cNetTranNumber)
{
	RETINFO ret				= {0};
	return ret;
}
...
RETINFO TicketSvt::svt_increase(PPURSETRADE p_purse, uint8_t * p_time, uint8_t * p_mac2 ,uint8_t paytype)
{
	RETINFO ret					= {0};
...
RETINFO TicketSvt::svt_decrease(PPURSETRADE p_purse, uint8_t u_pay_flag, uint8_t paytype)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	uint8_t tacType			= 0x06;
    uint8_t pay_type[1]	= {0};//支付方式
...
bool TicketSvt::last_trade_need_continue(uint8_t status_targ, uint16_t err_targ, uint16_t err_src)
{
	...
	do
	{
		...
		g_Record.log_out(0, level_normal, "status_targ:%d | init_status:%d |err_targ=%d |err_src=%d",status_targ,m_ticket_data.init_status,err_targ,err_src);

		if (!check_confirm(status_targ, m_ticket_data.init_status, err_targ, err_src))
			break;

		g_Record.log_out(0, level_normal, 
		"flag_trade_assist:%d | flag_metro:%d |flag_ctrl_record=%d",
		p_old_write->flag_trade_assist,
		p_old_write->flag_metro,
		p_old_write->flag_ctrl_record);
		...
		g_Record.log_out(0, level_normal, 
		"cfm_point.balance:%x | m_ticket_data.wallet_value=%x",
		cfm_point.balance,
		m_ticket_data.wallet_value);
		//判断钱包余额
		if (cfm_point.balance != m_ticket_data.wallet_value)
			break;

		g_Record.log_out(0, level_normal, 
		"m_ticket_data.charge_counter:%x | cfm_point.charge_counter=%x |m_ticket_data.total_trade_counter=%x|cfm_point.consume_counter=%x",
		m_ticket_data.charge_counter,
		cfm_point.charge_counter,
		m_ticket_data.total_trade_counter,
		cfm_point.consume_counter);
