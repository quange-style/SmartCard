...
void Download::file_mgr_itp(uint8_t * p_msg_file, uint8_t * p_data_send, uint16_t& len_send)
{
	...
	if (flag_start_end == 0)
	{
		recv_new_file_itp(p_msg_file, file_name, file_size, file_crc, p_data_send, len_send);
	}
	else if (flag_start_end == 1)
	{
		make_file_effect_itp(p_msg_file, file_name, file_size, file_crc, p_data_send, len_send);
	}    

}

void Download::make_file_effect_itp(uint8_t * p_msg_file, char * p_file_name, uint32_t size_file, uint32_t crc_file, uint8_t * p_data_send, uint16_t& len_send)
{
	RETINFO ret							= {0};

	ret.wErrCode = g_Parameter.config_param_itp(p_file_name);

	len_send = sizeof(ret);
	memcpy(p_data_send, &ret, len_send);
}

void Download::recv_new_file_itp(uint8_t * p_msg_file, char * p_file_name, uint32_t size_file, uint32_t crc_file, uint8_t * p_data_send, uint16_t& len_send)
{
	FILE * fp							= NULL;
	RETINFO ret							= {0};
	int received						= 0;
