...
//电子票二维码状态码
#define STATUS_FORBIDDEN            '0' //禁止使用
#define STATUS_INIT_SALE            '1' //初始发码
#define STATUS_FINISH_SALE          '2' //售票完成状态
#define STATUS_ENTRY                '3' //成功进站状态
#define STATUS_EXIT                 '4' //成功出站状态
#define STATUS_TICKET_GOT           '5' //取票完成
...
typedef enum
{
	...
	operUnloadTicketInfo,    //取交易结果交易
	operEntryCheck,
	operExitCheck,
	operClearEntry,          //清(取消)进站
	operClearExit,           //清(取消)出站
    operAddTicketResult,     //补票或罚款交易 用于单程票超程或者既超时又超程情况
    operAddEntry,            //补进站
    operAddExit,             //补出站
	operPenaltyRes,          //罚款交易
	operEIncr,				 //电子支付充值
...
typedef enum
{
	...
	mediBusCpu		= 0x04,
	mediBusMfEX		= 0x05,
	mediMetroCpuTct = 0x06,  			//地铁CPU乘次票(如单日票、多日票)
	mediJtbCpu      = 0x07,				//交通部CPU卡，如潇湘卡
	mediElecULTictet  = 0x08,           //电子单程票
	mediElecSJTTictet  = 0x09,          //预付费电子储值票
	mediElecPostPayTictet  = 0x10,      //后付费电子储值票
	mediThirdPay  = 0x11,
	mediElecOfflineTictet  = 0x99 		//特殊脱机电子票
} ETPMDM;
...
{
	SStatus_Exit = 1,       // 出闸
	SStatus_Exit_Tt = 2,    // 列车故障模式出站
	SStatus_Exit_Only = 3,  // 出站票
	...
	SStatus_Sale = 8,       // 发售
	SStatus_Es = 9,         // 预赋值
	SStatus_Refund = 10,    // 退款
	SStatus_Invalid = 11    // 票卡状态错误
} SimpleStatus;

// 电子票二维码码体中的网络状态
typedef enum
{
    Status_NET_Success = 1,       // 网络畅通
    Status_NET_Failure = 2,       // 网络中断
}NetStatus;

//扫码的串口类型
typedef enum
{
    TYPE_QR_COM4 = 1,       	 // usb转com4
    TYPE_QR_COM_OTHER = 2,       // 当前定义闸机的扫码头为其他
}TYPE_QR_COM;
