...
int svt_newhd_create_mf(void)
{
	...
	do
	{
		...
		//			;【密文替换卡片主控密钥, 注意更新后主控密钥替代传输密钥控制卡，清洗卡时需要认证卡主控而不是传输密钥了】
		//			00 84 0000 04		;judge:sw=9000
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			memcpy(szcmd,"\x84\xD4\x01\x00\x1C", 5);
			ulen = 5;
			memcpy(sztmp,"\x00\x00\x00",3);
			memcpy(sztmp+3, g_input.key.mf_mkey, 16);
			tmp_len = data_encrypt(19, sztmp, g_input.key.mf_tkey);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			ulen = 5;
			memcpy(sztmp,"\x01\x00\x00\x00\x00\x00\x03\x00",8);
			memcpy(sztmp+8, g_input.key.mf_mamk, 16);
			tmp_len = data_encrypt(24, sztmp, g_input.key.mf_mkey);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			memcpy(szcmd,"\x84\xD4\x00\x01\x24", 5);
			ulen = 5;
			memcpy(sztmp,"\x01\x01\x00\x00\x00\x00\x03\x00",8);
			memcpy(sztmp+8, g_input.key.mf_eak, 16);
			tmp_len = data_encrypt(24, sztmp, g_input.key.mf_mkey);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			memcpy(szcmd,"\x04\xE2\x00\x08\x17\x70\x11\x61\x0F\x4F\x09\xA0\x00\x00\x00\x03\x86\x98\x07\x01\x50\x02\x3F\x01",24);
			ulen = 24;
...
int svt_newhd_create_adf1(void)
{
	...
	do
	{
		...
		memcpy(szcmd, "\x80\xE0\x00\x02\x16\x3F\x01\x08\xB0\x00\x00\x00\x00\x00\xB9\x00\x01\x03\xA0\x00\x00\x00\x03\x86\x98\x07\x01",27);
		ulen = 27;
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			ulen = 5;
			memcpy(sztmp,"\x00\x00\x00",3);
			memcpy(sztmp+3, g_input.key.adf1_mkey, 16);
			//tmp_len = data_encrypt(19,sztmp,g_input.key.adf1_tkey);
			tmp_len = data_encrypt(19,sztmp,tempKey);

			memcpy(szcmd+ulen, sztmp, tmp_len);
			ulen += tmp_len;

			//MAC_3(g_input.key.adf1_tkey, ulen, szdata, szcmd, szcmd+ulen);
			MAC_3(tempKey, ulen, szdata, szcmd, szcmd+ulen);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			ulen = 5;
			memcpy(sztmp,"\x01\x00\x00\x00\x00\x00\x03\x00",8);
			memcpy(sztmp+8, g_input.key.adf1_mamk_00, 16);
			tmp_len = data_encrypt(24,sztmp,g_input.key.adf1_mkey);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			ulen = 5;
			memcpy(sztmp,"\x01\x01\x00\x00\x00\x00\x03\x00",8);
			memcpy(sztmp+8, g_input.key.adf1_mamk_01, 16);
			tmp_len = data_encrypt(24,sztmp,g_input.key.adf1_mkey);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			//szcmd[3]=keyidx++;
			ulen = 5;
			memcpy(sztmp,"\x01\x02\x00\x00\x00\x00\x03\x00",8);
			memcpy(sztmp+8, g_input.key.adf1_mamk_02, 16);
			tmp_len = data_encrypt(24,sztmp,g_input.key.adf1_mkey);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			//szcmd[3]=keyidx++;
			ulen = 5;
			memcpy(sztmp,"\x02\x01\x01\x00\x00\x00\x03\x00",8);
			memcpy(sztmp+8, g_input.key.adf1_mpk_0101, 16);
			tmp_len = data_encrypt(24,sztmp,g_input.key.adf1_mkey);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			//szcmd[3]=keyidx++;
			ulen = 5;
			memcpy(sztmp,"\x09\x01\x01\x00\x00\x00\x03\x00",8);
			memcpy(sztmp+8, g_input.key.adf1_mlk_0201, 16);
			tmp_len = data_encrypt(24,sztmp,g_input.key.adf1_mkey);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			//szcmd[3]=keyidx++;
			ulen = 5;
			memcpy(sztmp,"\x0C\x00\x00\x00\x00\x00\x03\x00",8);
			memcpy(sztmp+8, g_input.key.adf1_mtk, 16);
			tmp_len = data_encrypt(24,sztmp,g_input.key.adf1_mkey);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			memcpy(sztmp,"\x1F\x00\x00\x00\x00\x00\x03\x00",8);
			memcpy(sztmp+8, "\x12\x34\x56\xFF\xFF\xFF\xFF\xFF", 8);
			tmp_len = data_encrypt(16,sztmp,g_input.key.adf1_mkey);
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			// ??? 华虹的卡用的是 adf1_mamk_01 , 创建文件是否有不一致
			MAC_3(g_input.key.adf1_mamk_01, ulen, szdata, szcmd, szcmd+ulen);
		...
		//	00E2 00C0 17 FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 		;judge:sw=9000
		//	00E2 00C0 17 FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 		;judge:sw=9000
		memcpy(szcmd, "\x80\xE0\x00\x07\x0E\xEF\x18\x17\x0A\x00\x00\x00\x04\x00\x00\x00\x00\x00\x01",19);
		ulen = 19;
		//dbg_dumpmemory("create adf1 =",szcmd,ulen);
		respone_len = MifareProCom(ulen, szcmd, &respone_sw);
		//dbg_dumpmemory("create adf1 |=",szcmd,respone_len);
		...
		for(i=0; i<10; i++)
		{
			memcpy(szcmd, "\x00\xE2\x00\xC0\x17\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF",28);
			ulen = 28;
			//dbg_dumpmemory("create adf1 =",szcmd,ulen);
			respone_len = MifareProCom(ulen, szcmd, &respone_sw);
			if ((respone_len==0) || (0x9000!=respone_sw))
		...
		//	00E2 0080 17 FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 		;judge:sw=9000
		//	00E2 0080 17 FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 		;judge:sw=9000
		memcpy(szcmd, "\x80\xE0\x00\x07\x0E\xEF\x10\x17\x0A\x00\x00\x00\x04\x00\x00\x00\x00\x00\x01",19);
		ulen = 19;
		//dbg_dumpmemory("create adf1 =",szcmd,ulen);
		respone_len = MifareProCom(ulen, szcmd, &respone_sw);
		//dbg_dumpmemory("create adf1 |=",szcmd,respone_len);
		...
		for(i=0; i<10; i++)
		{
			memcpy(szcmd, "\x00\xE2\x00\x80\x17\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF",28);
			ulen = 28;
			//dbg_dumpmemory("create adf1 =",szcmd,ulen);
			respone_len = MifareProCom(ulen, szcmd, &respone_sw);
			if ((respone_len==0) || (0x9000!=respone_sw))
		...
		//	  00E2 00D0 17 FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF		  ;judge:sw=9000
		//	  00E2 00D0 17 FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF		  ;judge:sw=9000
		memcpy(szcmd, "\x80\xE0\x00\x07\x0E\xEF\x1A\x17\x0A\x00\x00\x00\x04\x00\x00\x00\x00\x00\x01",19);
		ulen = 19;
		//dbg_dumpmemory("create adf1 =",szcmd,ulen);
		respone_len = MifareProCom(ulen, szcmd, &respone_sw);
		//dbg_dumpmemory("create adf1 |=",szcmd,respone_len);
		...
		for(i=0; i<10; i++)
		{
			memcpy(szcmd, "\x00\xE2\x00\xD0\x17\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF",28);
			ulen = 28;
			//dbg_dumpmemory("create adf1 =",szcmd,ulen);
			respone_len = MifareProCom(ulen, szcmd, &respone_sw);
			if ((respone_len==0) || (0x9000!=respone_sw))
...
int svt_newhd_create_adf8(uint8_t * p_tk_type)
{
	int nresult					= 0;
	//unsigned char tmplen		= 0;
	unsigned char tmp_len;
	...
	do
	{
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			memcpy(sztmp+3, keytmp, 16);
			tmp_len = data_encrypt(19,sztmp,temp);//g_input.key.adf1_tkey

			memcpy(szcmd+ulen, sztmp, tmp_len);
			ulen += tmp_len;

			MAC_3(temp, ulen, szdata, szcmd, szcmd+ulen);//g_input.key.adf1_tkey
...
uint16_t es_svt_read(uint8_t *lpoutput)
{
	...
	do
	{
		...
		if(szcmd[0] == 0x00)
			szcmd[0] = 0x85;
		sprintf(sztmp, "%02x00", szcmd[0]);
		memcpy(lpresult->cardfactory, sztmp, 4);
...
uint16_t es_svt_init(uint8_t *lpinput, uint8_t *lpoutput)
{
	...
	do
	{
		...
		if (g_cardfactory==0x85 || g_cardfactory==0x00)
		{

			//g_Record.log_out(0, level_error, "svt_newhh_create_mf");
			nresult=svt_newhh_create_mf();
			if (nresult!= 0)
			{
				g_Record.log_out(0, level_error, "svt_newhh_create_mf(nresult=%d)",nresult);
				nret = nresult;
			...
			nresult = svt_newhh_create_adf1();
			//dbg_formatvar("svt_create_adf1=%d",nresult);
			if ( nresult!= 0)
			{
				g_Record.log_out(0, level_error, "svt_newhh_create_adf1(nresult=%d)",nresult);
				nret = 100+nresult;
				break;
			}

			nresult = svt_newhh_create_adf8(g_input.bcardtype);
			if (nresult != 0)
			{
				g_Record.log_out(0, level_error, "svt_newhh_create_adf8(nresult=%d)",nresult);
				nret = 100+nresult;
				break;
			}
		}
		else
		{
			...
			//g_Record.log_out(0, level_error, "svt_newhd_create_mf");
			nresult=svt_newhd_create_mf();
			//dbg_formatvar("svt_create_mf=%d",nresult);
			if (nresult!= 0)
			{
				g_Record.log_out(0, level_error, "svt_newhd_create_mf(nresult=%d)",nresult);
				nret = nresult;

				break;

			}
			nresult = svt_newhd_create_adf1();
			if ( nresult!= 0)
			{
				g_Record.log_out(0, level_error, "svt_newhd_create_adf1(nresult=%d)",nresult);
				nret = 100+nresult;
				break;
			}

			nresult = svt_newhd_create_adf8(g_input.bcardtype);
			if (nresult != 0)
			{
				g_Record.log_out(0, level_error, "svt_newhd_create_adf8(nresult=%d)",nresult);
				nret = 100+nresult;
				break;
			}

			nresult = svt_newhd_end();
			if (nresult != 0)
			{
				g_Record.log_out(0, level_error, "svt_newhd_end(nresult=%d)",nresult);
				nret = 100+nresult;
				break;
			}
		}
	...
	do
	{
		...
		if (g_input.balance>0)
		{
			...
			if (memcmp(balance_after, InitforLoad.Balance, 4)!=0)
			{
				...
				memcpy(moneybuf+6, "\x80\x00", 2);
				////dbg_dumpmemory("adf1_mlk_0201=",g_input.key.adf1_mlk_0201, 16);
				TripleDes(calMacKey, moneybuf, g_input.key.adf1_mlk_0201,
		...
		if (apdu_getrandom(szdata,4)==0)
		{
			...
			if (/*(g_input.balance>0) &&*/(memcmp
				...
				(g_input.inf.saleflag==1)&&(g_input.inf.cardtype[0] != 0x03))
			{
				inf[2] = 1;
			}
...
uint16_t es_svt_recode(uint8_t *lpinput, uint8_t *lpoutput)
{
	...
	do
	{
		...
		if (g_cardfactory==0x85 || g_cardfactory==0x00)
		{
			...
			ret = svt_newhh_create_adf8(g_input.bcardtype);
			if (ret != 0)
			{
				ret = 300+ret;
				break;
		...
		else
		{
			...
			ret = svt_newhd_create_adf8(g_input.bcardtype);
			if (ret != 0)
			{
				ret = 300+ret;
				break;
...
uint16_t es_svt_clear(uint8_t *lpinput, uint8_t *lpoutput)
{
	...
	do
	{
		...
		if (g_cardfactory==0x85|| g_cardfactory==0x00)
		{
			nresult = svt_newhh_reset();
		}
		else
		{
			nresult = svt_newhd_reset();
		}
...
RETINFO esapi_private(uint8_t *lpinput, uint8_t *lpoutput)
{
	...
	do
	{
		...
		if (svt_selectfile(0x1001) < 0)
		{
			...
			if (svt_selecfileaid(9,szdata) < 0)
			{
				//dbg_dumpmemory("ERR_CARD_READ=",g_input.key.adf1_mamk_02,16);
