...
int ble::ble_connect(uint8_t * address)
{
	...
	g_Record.log_out(0, level_error,"(3).connect intended device for mac address");
...
void ble::ble_close()
{
	...
	g_Record.log_out(0, level_error,"(5).close ble connect");
	uint8_t p_cmd_recved[200] = {0};
	uint8_t p_data_send[256] = {0};
	long msec = 0;
...
int ble::ble_getWriteBackData(ETYTKOPER operType,uint8_t *  p_base_add_info,uint16_t lengthData,uint8_t * revData)
{
	...
	//memcpy(revData + length, "0059", 4);//长度
	//length += 4;

	if(operType == operEntry)
		memcpy(revData + length, "1010", 4);//类型
	if(operType == operExit)
		memcpy(revData + length, "2010", 4);//类型
	if(operType == operGetETicket)
		memcpy(revData + length, "3000", 4);//类型
	else if(operType == operUnloadTicketInfo)
		memcpy(revData + length, "3010", 4);//类型
	length += 4;

	memcpy(revData + length, "00", 2);//验票通过
	length += 2;

	memcpy(revData + length, (uint8_t * )p_base_add_info, lengthData);
	length += lengthData;

	//dbg_dumpmemory("revData",revData,length);
...
void  ble::ble_DataExchange(uint8_t * data,int dataLength,uint16_t& len_data,uint8_t * revData)
{
	...
	g_Record.log_out(0, level_error,"(4).ble_DataExchange");
...
void ble::ble_scan(/*uint16_t& len_data*/)
{
	...
	g_Record.log_out(0, level_error,"(1).start to scan ble device");
...
int ble::ble_scanStop()
{
	...
	g_Record.log_out(0, level_error,"(2).stop to scan ble device");
...
int ble::ble_getServiceHandle(/*uint8_t * serviceNo, uint8_t * handleNo*/)
{
	...
	g_Record.log_out(0, level_error,"(serviceHandle).getServiceHandle before transmit");
	uint8_t p_cmd_recved[200] = {0};
...
void ble::ble_setSubscriber()
{
	...
	g_Record.log_out(0, level_error,"start to set ble Subscriber");
	uint8_t p_cmd_recved[200] = {0};
...
void ble::ble_setbaudrate()
{
	...
	g_Record.log_out(0, level_error,"(init).start to set ble tbaudrate 115200");
	uint8_t p_cmd_recved[200] = {0};
	...
	//for (i=0;i<ssbi_count;i++)
	{
		for(m =0; m < 20;m++)
		{
			...
			if((memcmp(ble::scanAllBleInf.ssbi[m].ble_name,ssbi_all[0].ble_name,16) ==0))
			{
				...
				g_Record.log_buffer("ble_name = ",ble::scanAllBleInf.ssbi[m].ble_name,16);
				g_Record.log_buffer("scanAllBleInf.mac_address = ",ble::scanAllBleInf.ssbi[m].mac_address,6);
				g_Record.log_buffer("ssbi_all.mac_address = ",ssbi_all[0].mac_address,6);
				if(memcmp(ble::scanAllBleInf.ssbi[m].mac_address, ssbi_all[0].mac_address, 6) !=0)
				{
					...
					g_Record.log_buffer("scanAllBleInf.mac_address2 = ",ble::scanAllBleInf.ssbi[m].mac_address,6);
					return 1;
				}else
				{
					return 0;
				...
				/*
				for (int j=i+1;j<ssbi_count;j++)
				{
					{
						memcpy(&ssbi_temp, ssbi_all + i, sizeof(SCAN_SINGLE_BLE_INF));
						dbg_formatvar("ssbi_temp(i=%d)|ble_name = %s",i,ssbi_temp.ble_name);
						dbg_dumpmemory("mac_address = ",ssbi_temp.mac_address,6);
						memcpy(ssbi_all + i, ssbi_all + j, sizeof(SCAN_SINGLE_BLE_INF));
						//dbg_formatvar("ssbi_all(i=%d)|ble_name = %s",i,ssbi_all[i].ble_name);
						//dbg_dumpmemory("mac_address = ",ssbi_temp.mac_address,6);
						memcpy(ssbi_all + j, &ssbi_temp, sizeof(SCAN_SINGLE_BLE_INF));
						dbg_formatvar("ssbi_all(i=%d)|ble_name = %s",i,ssbi_all[i].ble_name);
						dbg_dumpmemory("mac_address = ",ssbi_temp.mac_address,6);
		...
		for(m =0; m < 20;m++)
		{
			if(memcmp(ble::scanAllBleInf.ssbi[m].mac_address,"\x00\x00\x00\x00\x00\x00",6) != 0)
			{
				...
				g_Record.log_out(0,level_error,"m=[%d]",m);
				g_Record.log_buffer("ble_name = ",ble::scanAllBleInf.ssbi[m].ble_name,16);
				g_Record.log_buffer("mac_address = ",ble::scanAllBleInf.ssbi[m].mac_address,6);

				//g_Record.log_out(0, level_error,"===========================================================");
				//g_Record.log_buffer("scanAllBleInf.ssbi = ",ble::scanAllBleInf.ssbi[m],51);
			}
		}
...

	{
		for (uint32_t i=0; i<20; i++)
		{
			...
			//if (memcmp(sabi.ssbi[i].ble_name,qr_passcode+24,8)==0)//匹配小黄
			{
				...
				g_Record.log_buffer("mac_address:",sabi.ssbi[i].mac_address,6);
...
bool ble::ble_get_version(uint8_t * ble_version)
{
	...
	g_Record.log_buffer("ble_get_version p_data_send() = ", p_data_send, length);
	length = cmd_ble.cmd_send_ble(operType, p_cmd_recved, p_data_send, length, &msec);
	if(length == 0)
		flag = true;
	dbg_formatvar("msec:%ld",msec);
	g_Record.log_out(0, level_disaster,"ble_get_version use time = [%ld]", msec);
	return flag;
}
...
int ble::ble_all_oper(ETYTKOPER operType,uint8_t * passcode, uint8_t * p_eTicket_info, int lengthData)
{
	...
	if(length == 0)
		g_Record.log_out(0, level_error,"ble succ use counts:%d",(i + 1));
...
void ble::ble_reset(int flag)
{
	...
	dbg_dumpmemory("ble_reset p_data_send:", p_data_send, 6);
	g_Record.log_buffer("ble_reset p_data_send() = ", p_data_send, 6);
	length = cmd_ble.cmd_send_ble(operType,p_cmd_recved,p_data_send,length,&msec);
	dbg_formatvar("msec:%ld",msec);
	if(flag == 0)
		usleep(100 * 1000);//给100ms
	g_Record.log_out(0, level_disaster,"ble_reset use time = [%ld]", msec);
}
