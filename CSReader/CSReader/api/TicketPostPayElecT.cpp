...
TicketPostPayElecT::TicketPostPayElecT(char * p_current_sam, char * p_sam_posid, uint8_t * physic_info)
{
	...
	uint8_t * pMemoryPos = ((uint8_t *)this) + sizeof(TicketPostPayElecT);
...
uint16_t TicketPostPayElecT::metro_elec_read(P_POS_METRO_ELECT_INF p_mtr_inf)
{
	uint16_t u_ret			= 0;
...
RETINFO TicketPostPayElecT::analyse_free_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse, PBOMETICANALYZE p_eticanalyse)
{
	RETINFO ret				= {0};
	uint16_t err				= 0;
	uint16_t err1				= 0;
...
RETINFO TicketPostPayElecT::entry_gate(PENTRYGATE p_entry,PETICKETDEALINFO p_eticketdealinfo)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	uint16_t length = 0;
	...
	getDealDataPos(operEntry,length,m_ReadInf.p_eTicket_entry_info,m_ticket_data.qr_ticketdata.price);
	//dbg_formatvar("length:%d",length);
	//dbg_dumpmemory("p_eTicket_entry_info:",m_ReadInf.p_eTicket_entry_info,length);

	POS_METRO_ELECT_INF write_inf = file_need_write(operEntry, &m_ReadInf);
...
RETINFO TicketPostPayElecT::exit_gate(PPURSETRADE p_exit,PETICKETDEALINFO p_eticketdealinfo, MODE_EFFECT_FLAGS * p_degrade_falgs)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	...
	getDealDataPos(operExit,length,m_ReadInf.p_eTicket_exit_info,p_exit->lTradeAmount);

	POS_METRO_ELECT_INF write_inf = file_need_write(operExit, &m_ReadInf);
...
RETINFO TicketPostPayElecT::sale(PSJTSALE p_sjt_sale, uint16_t& len_sjt, POTHERSALE p_svt_sale, uint16_t& len_svt, TICKET_PARAM& ticket_prm)
{
	...
	POS_METRO_ELECT_INF write_inf = file_need_write(operSvtSale, &m_ReadInf);
...
RETINFO TicketPostPayElecT::getETicket(PGETETICKETRESPINFO p_geteticketrespinfo)
{
	...
	getDealDataPos(operGetETicket,length,m_ReadInf.p_eTicket_get_info,m_ticket_data.qr_ticketdata.price);
	...
	POS_METRO_ELECT_INF write_inf = file_need_write(operGetETicket, &m_ReadInf);
...
RETINFO TicketPostPayElecT::bom_update(PTICKETUPDATE p_update, uint8_t * p_entry_station)
{
	...
	POS_METRO_ELECT_INF write_inf = file_need_write(operUpdate, &m_ReadInf);
...
RETINFO TicketPostPayElecT::svt_increase(PPURSETRADE p_purse, uint8_t * p_time, uint8_t * p_mac2)
{
	RETINFO ret					= {0};
	return ret;
}

RETINFO TicketPostPayElecT::svt_decrease(PPURSETRADE p_purse, uint8_t u_pay_flag)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	...
	{
		POS_METRO_ELECT_INF write_inf = file_need_write(operDecr, &m_ReadInf);
...
POS_METRO_ELECT_INF TicketPostPayElecT::file_need_read(ETYTKOPER type, uint8_t * p_buffer)
{
	int ptr_buffer			= 0;
	POS_METRO_ELECT_INF ret_inf	= {0};
...
POS_METRO_ELECT_INF TicketPostPayElecT::file_need_write(ETYTKOPER type, P_POS_METRO_ELECT_INF p_read_inf)
{
	POS_METRO_ELECT_INF ret_inf = {0};
...
RETINFO TicketPostPayElecT::write_card(ETYTKOPER operType, uint8_t * p_eTicket_info, uint16_t length, uint8_t tac_type, long trade_amount, long& sam_seq, char * p_trade_tac)
{
	RETINFO ret			= {0};
	uint8_t u_recv[64]	= {0};
	char sz_tac[11]		= {0};
...
uint16_t TicketPostPayElecT::write_card(uint8_t tac_type, uint8_t * p_app_ctrl, int record_id, uint8_t lock_flag, uint8_t * p_file_data, long& sam_seq)
{
...
void TicketPostPayElecT::get_qrInfo(uint8_t * p_grInfo)
{
	...
	Api::sysFlag = flag%2;
	dbg_formatvar("flag = %d",Api::sysFlag);
	if(Api::sysFlag == 0)
		g_Record.log_out(0, level_disaster,"this is android phone");
	else
		g_Record.log_out(0, level_disaster,"this is apple phone");
		*/
	//g_Record.log_buffer("qr_readbuf_begin = ", Api::qr_readbuf,received+2);

	//memcpy(m_ticket_data.qr_bleMac, p_grInfo + 84, 12);
	//memcpy(m_ticket_data.qr_rfu, p_grInfo + 96, 20);

	//dbg_formatvar("qr_issueFlag = %s",m_ticket_data.qr_issueFlag);
	//dbg_formatvar("qr_version = %s",m_ticket_data.qr_version);
	//dbg_formatvar("qr_encMode = %s",m_ticket_data.qr_encMode);
	//dbg_formatvar("qr_passcode = %s",m_ticket_data.qr_passcode);
	//dbg_formatvar("qr_timestamp = %s",m_ticket_data.qr_timestamp);
	//dbg_formatvar("qr_bleMac = %s",m_ticket_data.qr_bleMac);
	//dbg_formatvar("qr_rfu = %s",m_ticket_data.qr_rfu);
	...
	memcpy(m_ticket_data.qr_ticketdata.mac, p_grInfo + 61, 8);
	g_Record.log_out(0, level_error,"get_qrInfo(mac=%s)", m_ticket_data.qr_ticketdata.mac);

	memcpy(m_ticket_data.qr_ticketdata.applyTime, p_grInfo + 69, 14);
	...
	if(m_ticket_data.qr_ticket_status == '1')//进站
	{
		//memcpy(m_ticket_data.qr_ticketdata.vervifyCode, p_grInfo + 125, 8);
		//g_Record.log_out(0, level_error,"get_qrInfo(vervifyCode=%s)", m_ticket_data.qr_ticketdata.vervifyCode);
		m_ticket_data.simple_status = SStatus_Exit;

	}else if(m_ticket_data.qr_ticket_status == '3'){
		memcpy(m_ticket_data.qr_ticketdata.startStation, p_grInfo + 83, 2);
		memcpy(m_ticket_data.qr_ticketdata.deviceType, p_grInfo + 85, 2);
		memcpy(m_ticket_data.qr_ticketdata.deviceCode, p_grInfo + 87, 3);
		memcpy(m_ticket_data.qr_ticketdata.startStation + 2, p_grInfo + 90, 2);

		g_Record.log_out(0, level_error,"get_qrInfo(deviceType=%s)", m_ticket_data.qr_ticketdata.deviceType);
		g_Record.log_out(0, level_error,"get_qrInfo(deviceCode=%s)", m_ticket_data.qr_ticketdata.deviceCode);
		g_Record.log_out(0, level_error,"get_qrInfo(startStation=%s)", m_ticket_data.qr_ticketdata.startStation);

		memcpy(m_ticket_data.qr_ticketdata.entryTime, p_grInfo + 92, 14);
		memcpy(m_ticket_data.qr_ticketdata.validOutTime, p_grInfo + 106, 14);
		//memcpy(m_ticket_data.qr_ticketdata.startStation, p_grInfo + 130, 4);
		...
		//dbg_formatvar("centerCode = %s",m_ticket_data.qr_ticketdata.centerCode);
		//dbg_formatvar("mac = %s",m_ticket_data.qr_ticketdata.mac);
		//dbg_formatvar("saleTime = %s",m_ticket_data.qr_ticketdata.saleTime);
		//dbg_formatvar("validDate = %s",m_ticket_data.qr_ticketdata.validDate);
		//dbg_formatvar("startStation = %s",m_ticket_data.qr_ticketdata.startStation);
		//dbg_formatvar("entryTime = %s",m_ticket_data.qr_ticketdata.entryTime);
		//dbg_formatvar("terminalStation = %s",m_ticket_data.qr_ticketdata.terminalStation);

		//memcpy(m_ticket_data.qr_info, Api::qr_readbuf, 120);
		...
		memcpy(m_ticket_data.qr_ticketdata.exitTime, p_grInfo + 115, 14);
...
RETINFO TicketPostPayElecT::lock(PTICKETLOCK p_lock)
{
	...
	POS_METRO_ELECT_INF write_inf = file_need_write(operLock, &m_ReadInf);
...
RETINFO TicketPostPayElecT::deffer(PTICKETDEFER p_deffer, TICKET_PARAM& ticket_prm)
{
	...
	POS_METRO_ELECT_INF write_inf = file_need_write(operDeffer, &m_ReadInf);
...
void TicketPostPayElecT::save_last_stack(ETYTKOPER operType, void * p_trade, size_t size_trade, void * p_written_inf, size_t size_written, bool save_to_file)
{
	...
	POS_METRO_ELECT_INF * p_old_write = (POS_METRO_ELECT_INF *)p_written_inf;
