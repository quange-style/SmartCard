...
TicketSJTElecT::TicketSJTElecT(void)
{

}

TicketSJTElecT::TicketSJTElecT(char * p_current_sam, char * p_sam_posid, uint8_t * physic_info)
{
	...
	uint8_t * pMemoryPos = ((uint8_t *)this) + sizeof(TicketSJTElecT);
...
uint16_t TicketSJTElecT::metro_elec_read(P_SJT_METRO_ELECT_INF p_mtr_inf)
{
	uint16_t u_ret			= 0;
...
RETINFO TicketSJTElecT::read_card(ETYTKOPER operType, TICKET_PARAM& ticket_prm, uint8_t * p_read_init/* = NULL*/)
{
	RETINFO ret				= {0};
	...
	do
	{
		...
		//dbg_formatvar("TicketSJTElecT::read_card");
...
uint8_t TicketSJTElecT::unified_status()
{
...
RETINFO TicketSJTElecT::analyse_common(uint8_t wk_area, MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PTICKETLOCK p_lock, uint16_t& lenlock, PBOMANALYZE p_analyse, char * p_mode)
{
	RETINFO ret				= {0};
	char mode				= '0';
...
RETINFO TicketSJTElecT::analyse_work_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse, PPURSETRADE p_purse, PBOMETICANALYZE p_eticanalyse)
{
	RETINFO ret				= {0};
	char exit_mode				= 0;
...
RETINFO TicketSJTElecT::analyse_free_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse, PBOMETICANALYZE p_eticanalyse)
{
	RETINFO ret				= {0};
	uint16_t err				= 0;
	uint16_t err1				= 0;
	...
	do
	{
		memcpy(m_ticket_data.fare_type, "\x02\x00", 2);//暂时当成传统业务中的单程票实体卡
		...
		//if (ret.wErrCode)	break;
		if (m_ticket_data.qr_ticketdata.balance < lowest_val)
		{
...
uint32_t TicketSJTElecT::add_oper_status(uint8_t wk_area, RETINFO ret, TICKET_PARAM * p_ticket_prm)
{
...
RETINFO TicketSJTElecT::entry_gate(PENTRYGATE p_entry,PETICKETDEALINFO p_eticketdealinfo)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	uint16_t length = 0;
	...
	getDealDataSjt(operEntry,length,m_ReadInf.p_eTicket_entry_info,m_ticket_data.qr_ticketdata.price);
	//dbg_formatvar("length:%d",length);
	//dbg_dumpmemory("p_eTicket_entry_info:",m_ReadInf.p_eTicket_entry_info,length);

	SJT_METRO_ELECT_INF write_inf = file_need_write(operEntry, &m_ReadInf);
	...
	{
		save_last_stack(operEntry, p_eticketdealinfo, sizeof(ETICKETDEALINFO), &write_inf, sizeof(write_inf), false);
	}*/
	if ((ret.wErrCode == ERR_BLE_WRITEBACK) || (ret.wErrCode == ERR_BLE_OTHER))
	{
...
RETINFO TicketSJTElecT::exit_gate(PPURSETRADE p_exit,PETICKETDEALINFO p_eticketdealinfo, MODE_EFFECT_FLAGS * p_degrade_falgs)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	...
	getDealDataSjt(operExit,length,m_ReadInf.p_eTicket_exit_info,p_exit->lTradeAmount);

	SJT_METRO_ELECT_INF write_inf = file_need_write(operExit, &m_ReadInf);
	...
	if ((ret.wErrCode == ERR_BLE_WRITEBACK) || (ret.wErrCode == ERR_BLE_OTHER))
	{
		save_last_stack(operExit, p_eticketdealinfo, sizeof(ETICKETDEALINFO), &write_inf, sizeof(write_inf), false);
	}*/
	if ((ret.wErrCode == ERR_BLE_WRITEBACK) || (ret.wErrCode == ERR_BLE_OTHER))
	{
...
RETINFO TicketSJTElecT::esale(PSJTSALE p_sjt_sale, uint16_t& len_sjt, PETICKETDEALINFO p_eticketdealinfo, uint16_t& len_eticket_sale, TICKET_PARAM& ticket_prm)
{
	RETINFO ret				= {0};
	return ret;
}

RETINFO TicketSJTElecT::thirdesale(PSJTSALE p_sjt_sale, uint16_t& len_sjt, PETICKETDEALINFO p_eticketdealinfo, uint16_t& len_eticket_sale, TICKET_PARAM& ticket_prm , char * cNetTranNumber)
{
	RETINFO ret				= {0};
	return ret;
}

RETINFO TicketSJTElecT::sale(PSJTSALE p_sjt_sale, uint16_t& len_sjt, POTHERSALE p_svt_sale, uint16_t& len_svt, TICKET_PARAM& ticket_prm)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	len_sjt					= 0;
	...
	SJT_METRO_ELECT_INF write_inf = file_need_write(operSvtSale, &m_ReadInf);
...
RETINFO TicketSJTElecT::getETicket(PGETETICKETRESPINFO p_geteticketrespinfo)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	uint16_t length = 0;
	...
	getDealDataSjt(operGetETicket,length,m_ReadInf.p_eTicket_get_info,m_ticket_data.qr_ticketdata.price);
	...
	SJT_METRO_ELECT_INF write_inf = file_need_write(operGetETicket, &m_ReadInf);
...
RETINFO TicketSJTElecT::bom_update(PTICKETUPDATE p_update, uint8_t * p_entry_station)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	long fare				= 0;

	SJT_METRO_ELECT_INF write_inf = file_need_write(operUpdate, &m_ReadInf);
...
RETINFO TicketSJTElecT::bom_refund(PDIRECTREFUND p_refund)
{
	RETINFO ret				= {0};
	return ret;
}

RETINFO TicketSJTElecT::svt_increase(PPURSETRADE p_purse, uint8_t * p_time, uint8_t * p_mac2)
{
	RETINFO ret					= {0};
	return ret;
}

RETINFO TicketSJTElecT::svt_decrease(PPURSETRADE p_purse, uint8_t u_pay_flag)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	...
	{
		SJT_METRO_ELECT_INF write_inf = file_need_write(operDecr, &m_ReadInf);
...
SJT_METRO_ELECT_INF TicketSJTElecT::file_need_read(ETYTKOPER type, uint8_t * p_buffer)
{
	int ptr_buffer			= 0;
	SJT_METRO_ELECT_INF ret_inf	= {0};
...
SJT_METRO_ELECT_INF TicketSJTElecT::file_need_write(ETYTKOPER type, P_SJT_METRO_ELECT_INF p_read_inf)
{
	SJT_METRO_ELECT_INF ret_inf = {0};
...
void TicketSJTElecT::get_issue_base(uint8_t * p_issue_base)
{
...
void TicketSJTElecT::get_public_base(uint8_t * p_public_base)
{
...
void TicketSJTElecT::get_owner_base(uint8_t * p_owner_base)
{
...
void TicketSJTElecT::get_manage_info(uint8_t * p_manage_info)
{
...
void TicketSJTElecT::get_wallet(uint8_t * p_wallet)
{
	if (p_wallet != NULL)
	{
...
void TicketSJTElecT::get_his_all(uint8_t * p_his_all)
{
	if (p_his_all != NULL)
	{
...
void TicketSJTElecT::get_trade_assist(uint8_t * p_trade_assist)
{
...
void TicketSJTElecT::get_metro(uint8_t * p_metro)
{
	if (p_metro != NULL)
	{
	}
...
void TicketSJTElecT::get_ctrl_record(uint8_t * p_ctrl_record)
{
...
void TicketSJTElecT::get_app_ctrl(uint8_t * p_app_ctrl)
{
...
void TicketSJTElecT::get_change_count(uint8_t * p_charge_count)
{
...
void TicketSJTElecT::get_consume_count(uint8_t * p_consume_count)
{
...
RETINFO TicketSJTElecT::write_card(ETYTKOPER operType, uint8_t * p_eTicket_info, uint16_t length, uint8_t tac_type, long trade_amount, long& sam_seq, char * p_trade_tac)
{
	RETINFO ret			= {0};
	uint8_t u_recv[64]	= {0};
	char sz_tac[11]		= {0};
	...
	do
	{
		...
		msec = time_use / 1000;
		g_Record.log_out(0, level_error,"setBauTest msec:%ld",msec);
		*/
...
RETINFO TicketSJTElecT::write_card_back(ETYTKOPER operType, uint8_t * p_eTicket_info, uint16_t length)
{
	RETINFO ret			= {0};
	uint8_t u_recv[64]	= {0};
	char sz_tac[11]		= {0};
...
uint16_t TicketSJTElecT::write_card(uint8_t tac_type, uint8_t * p_app_ctrl, int record_id, uint8_t lock_flag, uint8_t * p_file_data, long& sam_seq)
{
...
uint16_t TicketSJTElecT::set_trade_assist(uint8_t * p_trade_assist)
{
	uint16_t ret = 0;
	unsigned char u_recv[512] = {0};
...
uint16_t TicketSJTElecT::set_metro(uint8_t tac_type, uint8_t * p_metro, uint32_t lValue, uint8_t * p_respond)
{
	if (p_metro != NULL)
	{
...
uint16_t TicketSJTElecT::set_ctrl_record(uint8_t * p_ctrl_record)
{
	if (p_ctrl_record != NULL)
	{
	}
...
uint16_t TicketSJTElecT::set_app_ctrl(uint8_t * p_app_ctrl)
{
...
void TicketSJTElecT::get_qrInfo(uint8_t * p_grInfo)
{
	uint8_t temp[4]		= {0};
	...
	dbg_formatvar("ttemp = %02x",ttemp);

	dbg_formatvar("flag = %d",flag);

	Api::sysFlag = flag%2;

	dbg_formatvar("sysFlag = %d",Api::sysFlag);

	if(Api::sysFlag == 0)
		g_Record.log_out(0, level_disaster,"this is android phone");
	else
		g_Record.log_out(0, level_disaster,"this is apple phone");
	...
	//dbg_formatvar("qr_ticket_status = %02x",m_ticket_data.qr_ticket_status);


	memcpy(m_ticket_data.qr_ticketdata.ticketVersion, p_grInfo + 15, 2);
	memcpy(m_ticket_data.qr_ticketdata.subType, p_grInfo + 17, 2);

	//memcpy(m_ticket_data.qr_ticketdata.cardNo,p_grInfo + 61,10);
	...
	memcpy(m_ticket_data.qr_ticketdata.centerCode, p_grInfo + 29, 32);
	g_Record.log_out(0, level_error,"get_qrInfo(centerCode=%s)", m_ticket_data.qr_ticketdata.centerCode);

	Publics::string_to_hexs((char *)(p_grInfo + 61), 8, temp, 4);
	...
	//dbg_formatvar("m_ticket_data.qr_ticketdata.balance = %ld",m_ticket_data.qr_ticketdata.balance);
	g_Record.log_out(0, level_error,"get_qrInfo(balance=%ld)", m_ticket_data.qr_ticketdata.balance);

	memcpy(m_ticket_data.timeAndcenterCode, p_grInfo, 14);
	memcpy(m_ticket_data.timeAndcenterCode + 14, m_ticket_data.qr_ticketdata.centerCode, 32);


	if(m_ticket_data.qr_ticket_status == '1')//代表后面结构是进站
	{
		memcpy(m_ticket_data.qr_ticketdata.mac, p_grInfo + 85, 8);
		//memcpy(m_ticket_data.qr_ticketdata.vervifyCode, p_grInfo + 135, 8);
		...
		//memcpy(m_ticket_data.qr_info, Api::qr_readbuf, 101);

		m_ticket_data.simple_status = SStatus_Exit;

	}else if(m_ticket_data.qr_ticket_status == '3'){
		memcpy(m_ticket_data.qr_ticketdata.mac, p_grInfo + 69, 8);
		memcpy(m_ticket_data.qr_ticketdata.startStation, p_grInfo + 77, 4);
		memcpy(m_ticket_data.qr_ticketdata.entryTime, p_grInfo + 86, 14);
		memcpy(m_ticket_data.qr_ticketdata.validOutTime, p_grInfo + 100, 14);
	...
	}else if(m_ticket_data.qr_ticket_status == '4'){
		memcpy(m_ticket_data.qr_ticketdata.mac, p_grInfo + 69, 8);
		memcpy(m_ticket_data.qr_ticketdata.exitTime, p_grInfo + 109, 14);
...
void TicketSJTElecT::get_bleInfo(uint8_t * p_bleInfo)
{
	Json::Reader reader;
...
// 将状态格式化为简化状态
SimpleStatus TicketSJTElecT::get_simple_status(uint8_t init_status)
{
...
RETINFO TicketSJTElecT::format_history(uint8_t&	his_count, PHSSVT p_his_array, int his_max)
{
	RETINFO ret	= {0};
	HSSVT his_all[30];
	HSSVT his_temp;
...
bool TicketSJTElecT::format_history(uint8_t * p_his_buffer, HSSVT& his)
{
	char szSam[32]		= {0};
...
RETINFO TicketSJTElecT::lock(PTICKETLOCK p_lock)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;

	SJT_METRO_ELECT_INF write_inf = file_need_write(operLock, &m_ReadInf);
...
RETINFO TicketSJTElecT::deffer(PTICKETDEFER p_deffer, TICKET_PARAM& ticket_prm)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;

	SJT_METRO_ELECT_INF write_inf = file_need_write(operDeffer, &m_ReadInf);
...
RETINFO TicketSJTElecT::set_complex_file(ETYTKOPER operType,uint8_t tac_type, long lAmount,uint8_t * p_metro,uint8_t * p_recv)
{
	int pos_file_buf			= 0;
...
void TicketSJTElecT::save_last_stack(ETYTKOPER operType, void * p_trade, size_t size_trade, void * p_written_inf, size_t size_written, bool save_to_file)
{
	METRO_CPU_INF_W written_info = {0};
	SJT_METRO_ELECT_INF * p_old_write = (SJT_METRO_ELECT_INF *)p_written_inf;

	TicketBase::save_last_stack_eticket(operType, p_trade, size_trade, &written_info, sizeof(written_info), save_to_file);
}

bool TicketSJTElecT::last_trade_need_continue(uint8_t status_targ, uint16_t err_targ, uint16_t err_src)
{
	bool ret = false;
...
RETINFO TicketSJTElecT::continue_last_trade(void * p_trade)
{
	RETINFO ret			= {0};
	char sz_tac[11]		= {0};
	uint8_t u_recv[32]	= {0};
	...
	do
	{
		//CONFIRM_FACTOR_E factor_e = get_confirm_factor_e();
...
RETINFO TicketSJTElecT::data_version_upgrade()
{
	RETINFO ret		            = {0};
...
uint16_t TicketSJTElecT::read_wallet(long * p_wallet)
{
	uint16_t ret			= 0;
