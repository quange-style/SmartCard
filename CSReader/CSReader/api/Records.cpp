...
void Records::log_out_debug(uint16_t err_code, LOG_LEVEL lvl_appoint, const char * fmt, ...)
{
	return;
	int cnt								= 0;
	FILE * fp							= NULL;
...
void Records::log_buffer_debug(const char * p_discription, uint8_t * p_buffer, int size_buffer)
{
	return;
	char szTemp[8] = {0};
	char szLog[4096] = {0};
...
void Records::log_buffer(const char * p_discription, uint8_t * p_buffer, int size_buffer)
{
	char szTemp[8] = {0};
	char szLog[4096] = {0};
...
void Records::ergodic_dir_delete_target(char * p_path_dir, uint8_t * p_today, int save_time, int date_offset_filename)
{
	...
	char pathname_log[_POSIX_PATH_MAX] = {0};
	...
	if (access(p_path_dir, 0) == 0)
	{
		...
		while((ent = readdir(pDir)) != NULL)
		{
			...
			{
				unsigned long lsize = 1024 * 1024 * 50;//暂时给到50M  超过50M则会被删除
				if (GetLogDirSize(NAME_LOG_FOLDER) >= lsize)
				{
					//g_Record.log_out(0, level_error, "[GetLogDirSize] = %d",(int)GetLogDirSize(NAME_LOG_FOLDER));
					for (int n = -28;n<0;n++)
					{
						g_Record.log_out_debug(0, level_error, "[GetLogDirSize] = %d",(int)GetLogDirSize(NAME_LOG_FOLDER));
						memcpy(temp, p_today, 4);
						TimesEx::bcd_time_calculate(temp, T_DATE, n);
						sprintf(pathname_log, "%s%02X%02X%02X%02X%s", p_path_dir, temp[0],temp[1],temp[2],temp[3],NAME_LOG);
						g_Record.log_out_debug(0, level_error, "[pathname_log = %s, [n] = %d]",pathname_log,n);
						remove(pathname_log);
						g_Record.log_out_debug(0, level_error, "[GetDirSize111] = %ld,%d",GetLogDirSize(NAME_LOG_FOLDER),n);
						if (GetLogDirSize(NAME_LOG_FOLDER) < lsize)
						{
							//g_Record.log_out(0, level_error, "[GetDirSize222] = %ld,%d",GetLogDirSize(NAME_LOG_FOLDER),n);
							break;
						}
					}
				}
...
unsigned long Records::GetLogDirSize(char * path)
{
	unsigned long lDirSize = 0;
	DIR*	pdir;
	struct dirent*	pdirent;
	struct stat buf;
	char szPathFile[_POSIX_PATH_MAX] = {0};
	pdir = opendir(path);
	if(pdir == NULL)
	{
		//LogOut("[GetDirSize]can't open dir\n");
	}
	else
	{
		do
		{
			...
			if(pdirent == NULL)
			{
				//LogOut("[GetDirSize]Get Dir Size pdirent == NULL\n");
				break;
			}
			sprintf(szPathFile, path);
			strcat(szPathFile, "/");
			strcat(szPathFile, pdirent->d_name);
			if(stat(szPathFile, &buf)==0)
			{
				if(buf.st_mode == S_IFDIR)
				{
					//LogOut("[GetDirSize]Get Dir Size buf.st_mode == S_IFDIR, pdirent->d_name = %s\n",pdirent->d_name);
					//printf("[GetDirSize]Get Dir Size buf.st_mode == S_IFDIR, pdirent->d_name = %s \n",pdirent->d_name);
					continue;
				}
				else
				{
					if(memcmp(pdirent->d_name+8, ".log", 4) !=0)
					{
						sprintf(szPathFile, path);
						strcat(szPathFile, "/");
						strcat(szPathFile, pdirent->d_name);
						remove(szPathFile);
						//LogOut("[GetDirSize]Get Dir Size remove file = %s\n",szPathFile);
						//printf("[GetDirSize]Get Dir Size remove file = %s \n",szPathFile);
						continue;
					}
				}
			}
			...
			//----
			if((lDirSize < 4294967295) && (lDirSize >= 0))
			{
				lDirSize = lDirSize+buf.st_size;
				if(lDirSize<0||lDirSize>=4294967295)
				{
					lDirSize = 4294967295;
				}
			}
			...
			//LogOut("[GetDirSize]pdirent->d_name = %s   buf.st_size = %ld  st_blocksize = %ld",pdirent->d_name,buf.st_size,buf.st_blocksize);
			//printf("[GetDirSize]pdirent->d_name = %s   buf.st_size = %ld  st_blocksize = %ld \n",pdirent->d_name,buf.st_size,buf.st_blocksize);
		}while(1);
		closedir(pdir);
	}
	//LogOut("[GetDirSize]lDirSize = %ld  , path = %s g_ClearCount = %ld\n",lDirSize,path,g_ClearCount);
	//printf("[GetDirSize]lDirSize = %ld  , path = %s \n",lDirSize,path);
	//g_Record.log_out(0, level_error, "[GetDirSize]lDirSize = %ld, path = %s ",lDirSize,path);
