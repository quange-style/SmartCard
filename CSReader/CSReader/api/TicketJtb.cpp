...
TicketJtb::TicketJtb(char * p_current_sam, char * p_sam_posid, uint8_t * physic_info)
{
	...
	uint8_t * pMemoryPos = ((uint8_t *)this) + sizeof(TicketJtb);
	m_ticket_data.p_entry_info = new(pMemoryPos) TradeInfo;

	uint8_t * pMemoryPos1 = ((uint8_t *)this) + sizeof(TicketJtb) + sizeof(TradeInfo);
...
RETINFO TicketJtb::entry_gate(PENTRYGATE p_entry,PETICKETDEALINFO p_eticketdealinfo)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
RETINFO TicketJtb::exit_gate(PPURSETRADE p_exit,PETICKETDEALINFO p_eticketdealinfo, MODE_EFFECT_FLAGS * p_degrade_falgs)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
RETINFO TicketJtb::svt_increase(PPURSETRADE p_purse, uint8_t * p_time, uint8_t * p_mac2)
{
	RETINFO ret					= {0};
	return ret;
}

RETINFO TicketJtb::svt_decrease(PPURSETRADE p_purse, uint8_t u_pay_flag)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
