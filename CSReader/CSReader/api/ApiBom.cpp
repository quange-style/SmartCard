...
void Api::Bom_TicketAnalyze(uint8_t * param_stack, uint8_t * data_to_send, uint16_t& len_data)
{
	...
	dbg_formatvar("Bom_TicketAnalyze");
    ...
    do
    {
		...
		dbg_dumpmemory("analyse:locked_analyse_id:",locked_analyse_id,20);
	...
	dbg_formatvar("Bom_TicketAnalyze(lid=%s,work_area=%d)=(%04X,%02X)",
		ticket_data.logical_id, work_area, ret.wErrCode, ret.bNoticeCode);

	g_Record.log_out(ret.wErrCode, level_error,
		"Bom_TicketAnalyze(lid=%s,work_area=%d)=(%04X,%02X)",
		ticket_data.logical_id, work_area, ret.wErrCode, ret.bNoticeCode);
}
...
void Api::Bom_SvtIncrease2(uint8_t * param_stack, uint8_t * data_to_send, uint16_t& len_data)
{
	...
	uint8_t bTime[7]			= {0};
	uint8_t Mac2[4]				= {0};
	//dbg_formatvar("Bom_SvtIncrease2");
	do
	{
		ret.wErrCode = g_Parameter.device_initialized();
		if (ret.wErrCode != 0)
		...
		if((ret.wErrCode==ERR_CARD_WRITE)&&(ret.bNoticeCode == NTC_MUST_CONFIRM))
		{
			...
			//dbg_formatvar("******************************************************");
			//usleep(5000*1000);
			//g_Record.log_out(0, level_error, "read indoor TransactConfirm beign");
			...
			if(ret.wErrCode != 0)
			{
				//dbg_formatvar("continue_last_trade-sfailed");
				ret.wErrCode = ERR_CARD_WRITE;
				ret.bNoticeCode = NTC_MUST_CONFIRM;
				break;
			}
			//dbg_formatvar("continue_last_trade-succ");
...
void Api::Bom_TransactConfirm(uint8_t * param_stack, uint8_t * data_to_send, uint16_t& len_data)
{
	...
	TRANSACTCONFIRM tc;
	//dbg_formatvar("Bom_TransactConfirm");
	do
	{
		...
		{
			//dbg_formatvar("Bom_TransactConfirm-unlock_reader");
			//g_Record.log_out(0, level_error, "unlock_reader");
		...
		else
		{
			...
			{
				//dbg_formatvar("ERR_DIFFRENT_CARD");
				ret.wErrCode = ERR_DIFFRENT_CARD;
				break;
			}
			...
			{
				//dbg_formatvar("last_trade_need_continue");
				p_ticket->clear_confirm(ticket_data.logical_id, false, true);
				break;
			}
			//dbg_formatvar("continue_last_trade");
			ret = p_ticket->continue_last_trade(tc.bData);
			if (ret.wErrCode != 0)	break;
			//dbg_formatvar("size_of_confirm_trade");
