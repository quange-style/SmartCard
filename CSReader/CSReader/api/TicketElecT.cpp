...
TicketElecT::TicketElecT(char * p_current_sam, char * p_sam_posid, uint8_t * physic_info)
{
	...
	uint8_t * pMemoryPos = ((uint8_t *)this) + sizeof(TicketElecT);
...
RETINFO TicketElecT::read_card(ETYTKOPER operType, TICKET_PARAM& ticket_prm, uint8_t * p_read_init/* = NULL*/)
{
	...
	do
	{
		...
		//dbg_formatvar("TicketElecT::read_card");
...
RETINFO TicketElecT::analyse_common(uint8_t wk_area, MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PTICKETLOCK p_lock, uint16_t& lenlock, PBOMANALYZE p_analyse, char * p_mode)
{
    RETINFO ret				= {0};
	char mode				= '0';
...
RETINFO TicketElecT::analyse_work_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse, PPURSETRADE p_purse, PBOMETICANALYZE p_eticanalyse)
{
	RETINFO ret				= {0};
	char exit_mode				= 0;
	...
	memcpy(m_ticket_data.fare_type, "\x01\x00", 2);
...
RETINFO TicketElecT::analyse_free_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse, PBOMETICANALYZE p_eticanalyse)
{
	...
	do
	{
		memcpy(m_ticket_data.fare_type, "\x01\x00", 2);//暂时当成传统业务中的单程票实体卡

		memcpy(timenow, CmdSort::m_time_now, 7);
		memcpy(btimenow, timenow, 7);
		Publics::string_to_bcds(m_ticket_data.qr_ticketdata.saleTime, 14, btime, 7);
		Publics::string_to_bcds(m_ticket_data.qr_ticketdata.validDate, 8, etime, 4);

		//g_Record.log_buffer("time now: ", timenow, 7);
		//g_Record.log_buffer("sale Time: ", btime, 7);
		//g_Record.log_buffer("valid Date: ", etime, 7);

		TimesEx::bcd_time_calculate(btimenow, T_TIME, 0, 0, 10);
		if((memcmp(btimenow, btime, 7) < 0 ) || (memcmp(timenow, etime, 4) > 0))
		{
	...
	case operEntry:
		{
			...
			memcpy(dealData + length, "20180702", 8);//暂时不写
			length += 8;
...
RETINFO TicketElecT::entry_gate(PENTRYGATE p_entry,PETICKETDEALINFO p_eticketdealinfo)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
	uint16_t length = 0;
...
RETINFO TicketElecT::exit_gate(PPURSETRADE p_exit,PETICKETDEALINFO p_eticketdealinfo, MODE_EFFECT_FLAGS * p_degrade_falgs)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
RETINFO TicketElecT::svt_increase(PPURSETRADE p_purse, uint8_t * p_time, uint8_t * p_mac2)
{
	RETINFO ret					= {0};
	return ret;
}

RETINFO TicketElecT::svt_decrease(PPURSETRADE p_purse, uint8_t u_pay_flag)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
RETINFO TicketElecT::write_card(ETYTKOPER operType, uint8_t * p_eTicket_info, uint16_t length, uint8_t tac_type, long trade_amount, long& sam_seq, char * p_trade_tac)
{
	RETINFO ret			= {0};
	uint8_t u_recv[64]	= {0};
	char sz_tac[11]		= {0};
...
uint16_t TicketElecT::write_card(uint8_t tac_type, uint8_t * p_app_ctrl, int record_id, uint8_t lock_flag, uint8_t * p_file_data, long& sam_seq)
{
...
void TicketElecT::get_qrInfo(uint8_t * p_grInfo)
{
	...
	//dbg_formatvar("ticketVersion = %s",m_ticket_data.qr_ticketdata.ticketVersion);
	//dbg_formatvar("subType = %s",m_ticket_data.qr_ticketdata.subType);////
	//dbg_formatvar("cardNo = %s",m_ticket_data.qr_ticketdata.cardNo);

	Publics::string_to_hexs((char *)(p_grInfo + 29), 8, temp, 4);
	...
	Publics::string_to_hexs((char *)(p_grInfo + 37), 8, temp, 4);
	...
	//dbg_formatvar("m_ticket_data.qr_ticketdata.counter = %ld",m_ticket_data.qr_ticketdata.counter);

	memcpy(m_ticket_data.qr_ticketdata.centerCode, p_grInfo + 45, 32);
	g_Record.log_out(0, level_error,"get_qrInfo(centerCode=%s)", m_ticket_data.qr_ticketdata.centerCode);
	memcpy(m_ticket_data.qr_ticketdata.mac, p_grInfo + 77, 8);
	memcpy(m_ticket_data.qr_ticketdata.saleTime, p_grInfo + 85, 14);
	memcpy(m_ticket_data.qr_ticketdata.validDate, p_grInfo + 99, 8);
	...
	if(m_ticket_data.qr_ticket_status == '2')//售卡完成状态，代表后面结构是进站或者取票
	{
		memcpy(m_ticket_data.qr_ticketdata.startStation, p_grInfo + 107, 4);
		memcpy(m_ticket_data.qr_ticketdata.terminalStation, p_grInfo + 111, 4);
		...
		Publics::string_to_hexs((char *)(p_grInfo + 115), 8, temp, 4);
		...
		memcpy(m_ticket_data.qr_ticketdata.amount, p_grInfo + 123, 2);

		g_Record.log_out(0, level_error,"get_qrInfo(amount=%s)", m_ticket_data.qr_ticketdata.amount);

		Publics::string_to_hexs((char *)(p_grInfo + 125), 8, temp, 4);
		...
		//memcpy(m_ticket_data.qr_info, Api::qr_readbuf, 133);
		...
		memcpy(m_ticket_data.qr_ticketdata.startStation, p_grInfo + 107, 4);
		memcpy(m_ticket_data.qr_ticketdata.entryTime, p_grInfo + 111, 14);
		memcpy(m_ticket_data.qr_ticketdata.terminalStation, p_grInfo + 125, 4);
		...
		Publics::string_to_hexs((char *)(p_grInfo + 129), 8, temp, 4);
		m_ticket_data.qr_ticketdata.price = temp[0];
		m_ticket_data.qr_ticketdata.price <<= 8;
		m_ticket_data.qr_ticketdata.price += temp[1];
		m_ticket_data.qr_ticketdata.price <<= 8;
		...
		m_ticket_data.qr_ticketdata.price <<= 8;
		m_ticket_data.qr_ticketdata.price += temp[3];
		//dbg_formatvar("m_ticket_data.qr_ticketdata.price = %ld",m_ticket_data.qr_ticketdata.price);
		g_Record.log_out(0, level_error,"get_qrInfo(price=%ld)", m_ticket_data.qr_ticketdata.price);

		memcpy(m_ticket_data.qr_ticketdata.validOutTime, p_grInfo + 145, 14);
		g_Record.log_out(0, level_error,"get_qrInfo(validOutTime=%s)", m_ticket_data.qr_ticketdata.validOutTime);

		//memcpy(m_ticket_data.qr_info, Api::qr_readbuf, 161);
		...
		memcpy(m_ticket_data.qr_ticketdata.exitTime, p_grInfo + 129, 14);
