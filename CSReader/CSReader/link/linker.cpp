...
uint16_t sam_init(int sock_id, char * p_sam_id, char * p_tml_id)
{
	...
	if (sock_id == SAM_SOCK_1 || sock_id == SAM_SOCK_2)
	{
		//dbg_formatvar("sock_id:%d",sock_id);
	...
	else if (sock_id == SAM_SOCK_3)
	{
		ret = octm1_saminit(SAM_SOCK_3, sam_id);
		if (ret == 0)
		{
			sprintf(sz_sam_id, "0000%02X%02x%02x%02x%02x%02x", sam_id[0], sam_id[1], sam_id[2], sam_id[3], sam_id[4], sam_id[5]);
		}
	}
	else if(sock_id == SAM_SOCK_4)
	{
		ret = octjtb_saminit(sock_id, &len_sam, sam_id, &len_tml, tml_id);
		if (ret == 0)
		{
			pos = 16 - len_sam * 2;
			memset(sz_sam_id, '0', pos);
