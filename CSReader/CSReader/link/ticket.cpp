...
unsigned short search_third_card_ex(unsigned char *lpcardno, unsigned char& type)
{
	...
	if((Api::qr_readbuf[0] == 0x51)&&(Api::qr_readbuf[1] == 0x52))
	{
		...
		//dbg_dumpmemory("tmp_package_time:",tmp_package_time,7);
		//g_Record.log_buffer("tmp_package_time = ", tmp_package_time,7);
		//dbg_dumpmemory("get_package_time:",Api::get_package_time,7);
		//g_Record.log_buffer("get_package_time = ", Api::get_package_time,7);
		TimesEx::bcd_time_calculate(tmp_package_time, T_TIME, 0, 0, 0, 2);//范围两秒内的都认
		//dbg_dumpmemory("tmp_package_time+2:",tmp_package_time,7);
		//g_Record.log_buffer("tmp_package_time+2() = ", tmp_package_time, 7);
...
void get_qrHeadInfo(uint8_t * p_grInfo, char * encMode)
{
	...
	if(Api::sysFlag == 0)
		...
		*/
	//dbg_formatvar("encMode = %s",encMode);

	//g_Record.log_out(0, level_error,"get_qrInfo(qr_passcode=%s)", m_ticket_data.qr_passcode);
...
void get_qrClearInfo(uint8_t * p_grInfo, uint8_t * key)
{
	...
	//dbg_dumpmemory("clear_qr_readbuf--",Api::clear_qr_readbuf ,dataLen * 2);

	/*
	memcpy(m_ticket_data.qr_issueFlag, p_grInfo + 2, 4);
	memcpy(m_ticket_data.qr_version, p_grInfo + 6, 2);
	memcpy(m_ticket_data.qr_encMode, p_grInfo + 8, 2);
	memcpy(m_ticket_data.qr_passcode, p_grInfo + 10, 32);
	memcpy(Api::qr_passcode, p_grInfo + 10, 32);
	memcpy(m_ticket_data.qr_timestamp, p_grInfo + 42, 14);

	memcpy(Api::qr_passcode, m_ticket_data.qr_passcode, 32);//将读取到的PASSCODE保存在一个全局变量里面

	memcpy(encMode, m_ticket_data.qr_encMode, 2);
	dbg_formatvar("encMode = %s",encMode);
...
unsigned short search_card_ex(unsigned char *lpcardno, unsigned char& type)
{
	...
	if((Api::qr_readbuf[0] == 0x51)&&(Api::qr_readbuf[1] == 0x52))
	{
		//isValidity = check_ElecT_isValidty();
		if((Api::qr_readbuf[2 + 7]==0x43)&&(Api::qr_readbuf[3 + 7]==0x53)&&(Api::qr_readbuf[4 + 7]==0x4D)&&(Api::qr_readbuf[5 + 7]==0x47))
		{
		...
		{
			g_Record.log_buffer("QR is invalid, the wrong qr = ", Api::qr_readbuf,187);
			ret = ERR_ETICKET_QR_INVALID;
		}
	}
	else
