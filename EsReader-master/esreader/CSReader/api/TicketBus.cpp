...
uint16_t TicketBus::bus_svt_read(P_BUS_CPU_INF p_bus_inf)
{
	...
	do
	{
		...
		{
			g_Record.log_out(0, level_error, "svt_selectfile");
			u_ret = ERR_CARD_READ;
			break;
		}
		...
		if (p_bus_inf->p_issue_base != NULL)
		{
			...
			{
				g_Record.log_out(0, level_error, "svt_readbinary");
				u_ret = ERR_CARD_READ;
				break;
			}
		...
		if (p_bus_inf->p_app_index != NULL)
		{
			...
			{
				g_Record.log_out(0, level_error, "svt_readbinary-06");
				u_ret = ERR_CARD_READ;
				break;
			}
		}
		...
		{
			g_Record.log_out(0, level_error, "svt_selectfile-ADF1");
			u_ret = ERR_CARD_READ;
			break;
		}
		...
		if (p_bus_inf->p_public_base != NULL)
		{
			...
			{
				g_Record.log_out(0, level_error, "svt_readbinary-15");
				u_ret = ERR_CARD_READ;
				break;
			}
		}
		...
		if (p_bus_inf->p_owner != NULL)
		{
			...
			{
				g_Record.log_out(0, level_error, "svt_readbinary-16");
				u_ret = ERR_CARD_READ;
				break;
			}
		}
		...
		{
			if (svt_getbalance(p_bus_inf->p_wallet) < 0)
			{
				g_Record.log_out(0, level_error, "svt_getbalance");
				u_ret = ERR_CARD_READ;
				break;
			}
		}
		...
		if (p_bus_inf->p_his_all != NULL)
		{
			...
			{
				g_Record.log_out(0, level_error, "svt_readhistory-18");
				u_ret = ERR_CARD_READ;
				break;
			}
			...
			{
				g_Record.log_out(0, level_error, "svt_readhistory-10");
				u_ret = ERR_CARD_READ;
				break;
			}
			if (svt_readhistory(0x1A, 1, 10, p_bus_inf->p_his_all + 0x17 * 20) < 0)		// 圈存
			{
				g_Record.log_out(0, level_error, "svt_readhistory-1a");
				u_ret = ERR_CARD_READ;
				break;
			}
		}
		...
		if (p_bus_inf->p_metro != NULL)
		{
			...
			{
				g_Record.log_out(0, level_error, "svt_readrecord-17");
				u_ret = ERR_CARD_READ;
				break;
			}
		}
		...
		if (p_bus_inf->p_consume_count != NULL)
		{
			...
			{
				g_Record.log_out(0, level_error, "svt_getofflineserial");
				u_ret = ERR_CARD_READ;
				break;
			}
		}
...
RETINFO TicketBus::analyse_work_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse, PPURSETRADE p_purse)
{
	RETINFO ret				= {0};
...
RETINFO TicketBus::analyse_free_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse)
{
	RETINFO ret				= {0};
...
RETINFO TicketBus::entry_gate(PENTRYGATE p_entry)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
RETINFO TicketBus::exit_gate(PPURSETRADE p_exit, MODE_EFFECT_FLAGS * p_degrade_falgs)
{
	RETINFO ret				= {0};
	long lSamSeq			= 0;
...
uint16_t TicketBus::oct_get_lock_flag()
{
	...
	{
		g_Record.log_out(0, level_error, "MifareProCom-00B203D400");
