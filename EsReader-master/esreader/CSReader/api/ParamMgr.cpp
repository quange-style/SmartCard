...
ParamMgr::PRM_FUNC_LIST ParamMgr::m_prm_for_reader[] =
{
    ...
    {PRM_0602_SECTION_BLACK_M,	&ParamMgr::load_mtr_section_balck},
    {PRM_0603_SINGLE_BLACK_O,	&ParamMgr::load_oct_single_black},
    {PRM_0604_SECTION_BLACK_O,	&ParamMgr::load_oct_section_balck},
...
ParamMgr::~ParamMgr(void)
{
    ...
    unload_mtr_section_balck();
    unload_oct_single_black();
    unload_oct_section_balck();
...
bool ParamMgr::prm_config_unison()
{
    ...
    {
        if (m_prm_config.find(m_prm_for_reader[i].prm_type) == m_prm_config.end() && (m_prm_for_reader[i].prm_type & 0xF0F0) != 0x9020)
        {
            ret = false;
		...
		while (fp.read_line(sz_read, sizeof(sz_read), read_cnt))
		{
			if (prm_section_match(sz_read, 0x0401))
			{
				...
				if (!only_check)
				{
					...
					{
						//dbg_formatvar("sz_read=%s",sz_read);
						//g_Record.log_out(0, level_error, "sz_read=%s",sz_read);
					...
					{
						g_Record.log_out(0, level_error, "loadcfparam");
						g_Record.log_out(0, level_error, "sz_read=%s",sz_read);
						cffrZone.cf_station_id = Publics::string_to_bcd<uint16_t>(sz_read + 5, 4);
						cffrZone.cf_exit_station_id = Publics::string_to_bcd<uint16_t>(sz_read + 9, 4);
...
int ParamMgr::load_mtr_section_balck(const char * p_prm_path, bool only_check/* = false*/)
{
    char sz_read[128]		= {0};
...
void ParamMgr::unload_mtr_section_balck()
{
...
int ParamMgr::load_oct_section_balck(const char * p_prm_path, bool only_check/* = false*/)
{
    char sz_read[128]		= {0};
    char sz_start_id[21]	= {0};
    char sz_end_id[21]		= {0};
...
void ParamMgr::unload_oct_section_balck()
{
...
int ParamMgr::query_highest_fare(uint8_t * p_time_now, long * p_fare_highest_ini)
{
	...
	{
		//dbg_formatvar
		//dbg_dumpmemory("query_highest_fare");
		//fare_lowest_actual = 200;
...
uint16_t ParamMgr::query_min_fare_zone(uint16_t entry_station_id, uint8_t& fare_zone)
{
	...
	{
		if ((m_prm_fare.vec_cf_zone[i].cf_station_id == entry_station_id)&&(m_prm_fare.vec_cf_zone[i].cf_exit_station_id == entry_station_id))
		{
...
uint16_t ParamMgr::config_param(char * p_name_prm)
{
	...
	do
	{
		...
		if ((prm_type & 0xF0F0) == 0x9020)
		{
			...
			if (!PackageMgr::ExcuteShell(source_path))
			{
				ret = ERR_FILE_ACCESS;
				break;
			}
