...
TicketBM::TicketBM(char * p_current_sam, char * p_sam_posid, uint8_t * physic_info)
{
	...
	uint8_t * pMemoryPos = ((uint8_t *)this) + sizeof(TicketBM);
...
RETINFO TicketBM::read_card(ETYTKOPER operType, TICKET_PARAM& ticket_prm, uint8_t * p_read_init)
{
	...
	do
	{
		...
		{
			g_Record.log_out(ret.wErrCode, level_error, "octm1_getdata2");
...
RETINFO TicketBM::analyse_work_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse, PPURSETRADE p_purse)
{
	RETINFO ret				= {0};
...
RETINFO TicketBM::analyse_free_zone(MODE_EFFECT_FLAGS * p_degrade_falgs, TICKET_PARAM * p_ticket_prm, PBOMANALYZE p_analyse)
{
	RETINFO ret				= {0};
...
RETINFO TicketBM::entry_gate(PENTRYGATE p_entry)
{
	RETINFO ret				= {0};
...
RETINFO TicketBM::exit_gate(PPURSETRADE p_exit, MODE_EFFECT_FLAGS * p_degrade_falgs)
{
	RETINFO ret				= {0};
	WRITE_BUS_M1 wrt_inf	= {0};
...
RETINFO TicketBM::svt_decrease(PPURSETRADE p_purse, uint8_t u_pay_flag)
{
	RETINFO ret				= {0};
	WRITE_BUS_M1 wrt_inf	= {0};
...
bool TicketBM::format_history(uint8_t * p_his_buffer, HSSVT& his)
{
	...
	// POS 机编号
	sprintf(his.cSAMID, "%08X%02X%02X%02X%02X", 0, p_his_buffer[3], p_his_buffer[4], p_his_buffer[5], p_his_buffer[6]);
