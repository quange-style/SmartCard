...
void CDlgSamAct::OnBnClickedBnSamAct()
{
	...
	if (g_bDeviceType == 2)//tvm
		((CCSTesterDlg*)AfxGetMainWnd())->DispResult(struct_Null, NULL, 0, "TVM: SAM卡激活", g_liStart, g_liEnd);
	else//bom
		((CCSTesterDlg*)AfxGetMainWnd())->DispResult(struct_Null, NULL, 0, "BOM: SAM卡激活", g_liStart, g_liEnd);
